...
public class Raids {
    ...
    private static final Map<String, Integer> raidPlayers = Maps.newConcurrentMap();
    private final Map<String, Integer> activeRoom = Maps.newConcurrentMap();
    private List<RaidsRank> ranks = null;
    private static int groupPoints;


    public static void sendRaidInfo() {//sORT THIS CAM todo
        for(Player player : getPlayers()) {
            ...
            if (Boundary.isIn(player, Boundary.RAIDS_LOBBY_ENTRANCE)) {
                return;
            }
            ...
            int totalMembers = raidPlayers.size();
            player.getPA().sendFrame126("Party Size: " + totalMembers, 24001);
            player.getPA().sendFrame126("Points: " + groupPoints, 24002);
            player.getPA().sendFrame126("Damage:" + getDamage(player), 24003);
            player.getPA().sendFrame126("", 24004);
        }
    }
...
//
    ...
    public void startRaid(List<Player> players, boolean party) {
        ...
        for (Player lobbyPlayer : players) {
            ...
            lobbyPlayer.sendMessage("@pur@The raid has now started! Good Luck! type ::leaveraids to leave!");
            lobbyPlayer.sendMessage("@red@[TEMP] @blu@If you get stuck in a wall, type @red@::stuckraids @blu@to be sent back to @red@room 1!");
        }
    ...
    public void leaveGame(Player player) {
        ...
        //player.getPA().movePlayer(3034, 6067, 0);// old zone
        player.getPA().movePlayer(1245, 3562, 0);
        player.setRaidsInstance(null);
    ...
    public void giveReward(Player player, Boolean kronosReward) {
        ...
        if (getDamage(player) < RAIDS_DAMAGE_FOR_REWARD) {
            player.sendMessage("@pur@You didn't do enough damage to earn a reward, you must do at least @red@" + RAIDS_DAMAGE_FOR_REWARD + " damage!");
            return;
        }
        ...
        player.sendMessage("@red@"+player.getDisplayName()+" @pur@Your place was @red@#" + myRank + " @pur@with @red@" + getDamage(player) + "@pur@ damage done.");
        if (myRank > 5) {
            myRank = myRank + 4;
        }
        if (myRank > 24) {
        ...
        if (player.getItems().playerHasItem(21046)) {
            ...
            player.sendMessage("@pur@You sacrifice your @red@tablet @pur@for an increased @red@drop rate.");
        ...
        if (!kronosReward) {
            if (player.raidCount == 25) {
                ...
                PlayerHandler.executeGlobalMessage("@red@" + player.getDisplayName() + "@pur@ has completed @red@25 Raids@pur@ and obtained the @red@Xeric\'s Guard Cape!");
            }
            if (player.raidCount == 50) {
                player.getItems().addItemUnderAnyCircumstance(22390, 1);
                PlayerHandler.executeGlobalMessage("@red@" + player.getDisplayName() + "@pur@ has completed @red@50 Raids@pur@ and obtained the @red@Xeric\'s Warrior Cape!");
            }
            if (player.raidCount == 100) {
                player.getItems().addItemUnderAnyCircumstance(22392, 1);
                PlayerHandler.executeGlobalMessage("@red@" + player.getDisplayName() + "@pur@ has completed @red@100 Raids@pur@ and obtained the @red@Xeric\'s Sentinel Cape!");
            }
            if (player.raidCount == 250) {
                player.getItems().addItemUnderAnyCircumstance(22394, 1);
                PlayerHandler.executeGlobalMessage("@red@" + player.getDisplayName() + "@pur@ has completed @red@250 Raids@pur@ and obtained the @red@Xeric\'s General Cape!");
            }
            if (player.raidCount == 500) {
                player.getItems().addItemUnderAnyCircumstance(22396, 1);
                PlayerHandler.executeGlobalMessage("@red@" + player.getDisplayName() + "@pur@ has completed @red@500 Raids@pur@ and obtained the @red@Xeric\'s Champions Cape!");
            }
        }
    }
...
//			Server.getGlobalObjects().remove(new GlobalObject(29881, 3220, 5738, currentHeight, 3, 10).setInstance(instance));
			...
			getPlayers().stream().forEach(player -> {
				...
				player.sendMessage("@red@"+player.getLoginName()+" @pur@congratulations you have completed @red@ the chambers of xeric!");
				player.sendMessage("@pur@Please go up the stairs to get your rewards." );
			});
			return;
		...
		case OLM_RIGHT_HAND:
			...
			if(leftHand) {
				...
				getPlayers().stream().forEach(player ->	player.sendMessage("@pur@ You have defeated @red@one of The Great Olm's hands @pur@destroy the other one quickly!"));
			}
			...
			.forEach(otherPlr -> {
				...
				if(leftHand) {
					otherPlr.sendMessage("@pur@ You have defeated @red@both of The Great Olm's hands@pur@ he is now vulnerable.");
				} else {
					otherPlr.sendMessage("@pur@ You have defeated @red@one of The Great Olm's hands@pur@ destroy the other one quickly!");
				}
			});
		...
		if(killer != null) {
			...
			killer.sendMessage("@pur@You receive @red@"+ randomPoints +" points @pur@from killing this monster.");
			killer.sendMessage("@pur@You now have @red@"+ newPoints +" points.");
		}
		if(mobAmount <= 0) {
			getPlayers().stream().forEach(player ->	player.sendMessage("@pur@The room has been cleared and you are free to pass."));
			roomSpawned = false;
		}else {
			getPlayers().stream().forEach(player ->	player.sendMessage("@pur@There are @red@"+ mobAmount+" enemies @pur@remaining."));
		}
	}
	...
	public boolean handleObjectClick(Player player, int objectId, int x, int y) {
		...
		case 29778:
			...
			//player.sendMessage("@pur@You receive your @red@reward." );
			player.sendMessage("@pur@You have completed @red@"+player.raidCount+" raids." );
			leaveGame(player);
			break;
