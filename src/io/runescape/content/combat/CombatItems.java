package io.runescape.content.combat;
...
import io.runescape.content.items.OrnamentedItem;
...
public class CombatItems {
	...
	public boolean usingCrystalBow() {
		return c.playerEquipment[Player.playerWeapon] >= 4212 && c.playerEquipment[Player.playerWeapon] <= 4223 || c.playerEquipment[Player.playerWeapon] == 20999  || c.playerEquipment[Player.playerWeapon] == 25867;
	}
	...
	public boolean usingBlowPipe() {
		return c.playerEquipment[Player.playerWeapon] == Items.TOXIC_BLOWPIPE || c.playerEquipment[Player.playerWeapon] == 24106;
	}
	...
	public boolean usingAssembler() {
		return c.getItems().isWearingItem(Items.ASSEMBLER_MAX_CAPE)
		        ...
		        || c.getItems().isWearingItem(34037);//DEATH CAPE
	}
	...
	public void checkBlowpipe() {
		...
		if (c.getItems().isWearingItem(24_106, Player.playerWeapon)) {
			...
			if (c.getToxicBlowpipeAmmoAmount() <= 0 && c.getToxicBlowpipeCharge() <= 0) {
				c.sendMessage("Your vorkath blowpipe has lost all charge.");
				c.getItems().equipItem(-1, 0, 3);
				c.getItems().addItemUnderAnyCircumstance(24107, 1);
				c.setToxicBlowpipeAmmo(0);
				c.setToxicBlowpipeAmmoAmount(0);
				c.setToxicBlowpipeCharge(0);
			}
