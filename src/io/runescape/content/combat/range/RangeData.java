package io.runescape.content.combat.range;
...
import io.runescape.content.combat.effects.damageeffect.impl.bolts.DiamondBoltSpecial;
import io.runescape.content.combat.effects.damageeffect.impl.bolts.DragonBoltSpecial;
import io.runescape.content.combat.effects.damageeffect.impl.bolts.EmeraldBoltSpecial;
import io.runescape.content.combat.effects.damageeffect.impl.bolts.JadeBoltSpecial;
import io.runescape.content.combat.effects.damageeffect.impl.bolts.OnyxBoltSpecial;
import io.runescape.content.combat.effects.damageeffect.impl.bolts.OpalBoltSpecial;
import io.runescape.content.combat.effects.damageeffect.impl.bolts.PearlBoltSpecial;
import io.runescape.content.combat.effects.damageeffect.impl.bolts.RubyBoltSpecial;
import io.runescape.content.combat.effects.damageeffect.impl.bolts.SapphireBoltSpecial;
import io.runescape.content.combat.effects.damageeffect.impl.bolts.TopazBoltSpecial;
...
public class RangeData {
    ...
    public static final int[] BOWS = { 19481, 19478, 12788, 9185, 11785, 21012, 24119, 839, 845, 847, 851, 855, 859, 841, 843, 849,
            853, 857, 12424, 861, 4212, 4214, 4215, 12765, 12766, 12767, 12768, 11235, 4216, 4217, 4218, 4219, 4220,
            4221, 4222, 4223, 4734, 6724, 20997, 21902, 22550, 24111, 20999, 23145, 26374, 25867 };
    ...
    public static final int[] ARROWS = { 9341, 4160, 11959, 10033, 10034, 882, 883, 884, 885, 886, 887, 888, 889, 890, 891,
            ...
            21974};
    public static final int[] CRYSTAL_BOWS = { 4212, 4214, 4215, 4216, 4217, 4218, 4219, 4220, 4221, 4222, 4223, 20999, 25867};

    public static final int[] NO_ARROW_DROP = { 11959, 10033, 10034, 4212, 22550, 4214, 4215, 4216, 4217, 4218, 4219, 4220, 4221,
            4222, 4223, 4734, 4934, 4935, 4936, 4937, 22550, 20999, 25867 };
    ...
    public static final int[] OTHER_RANGE_WEAPONS = { 11959, 10033, 10034, 800, 801, 802, 803, 804, 805, 20849, 806, 807, 808,
            ...
            6522, 11230, 11231, 11233,11234, 22804, 22634, 21318, 19484, 25849 };
	...
	public static int getRangeStartGFX(Player c) {
		...
		int[][] data = {
				...
				// DARTS
				{ 806, 232 }, { 807, 233 }, { 808, 234 }, { 809, 235 }, { 810, 236 }, { 811, 237 }, { 11230, 237 }, { 25849, 237 },
	...
	public static int getRangeProjectileGFX(Player c) {
		...
		if (c.getItems().isWearingItem(12926)) {
			...
			final int[][] DARTS = { { 806, 226 }, { 807, 227 }, { 808, 228 }, { 809, 229 }, { 810, 230 }, { 811, 231 }, { 11230, 231 }, { 25849, 231 } };
		...
		// Vorkath Blowpipe
		if (c.getItems().isWearingItem(24106)) {
			Optional<Integer> original = PoisonedWeapon.getOriginal(c.getToxicBlowpipeAmmo());
			int ammo = original.orElseGet(c::getToxicBlowpipeAmmo);
			final int[][] DARTS = { { 806, 226 }, { 807, 227 }, { 808, 228 }, { 809, 229 }, { 810, 230 }, { 811, 231 }, { 11230, 231 }, { 25849, 231 } };
			for (int index = 0; index < DARTS.length; index++) {
		...
		// Crossbows
		if (c.playerEquipment[Player.playerWeapon] == 9185 || c.playerEquipment[Player.playerWeapon] == 11785 || c.playerEquipment[c.playerWeapon] == 23145 || c.playerEquipment[Player.playerWeapon] == 21012 || c.playerEquipment[Player.playerWeapon] == 26374|| c.playerEquipment[Player.playerWeapon] == 24119
		...
		int[][] data = {
				...
				// DARTS
				{ 806, 226 }, { 807, 227 }, { 808, 228 }, { 809, 229 }, { 810, 230 }, { 811, 231 }, { 11230, 231 }, { 25849, 231 },
				...
				{ 4222, 249 }, { 4223, 249 }, { 20999, 249 }, { 25867, 249 },};
		for (int l = 0; l < data.length; l++) {
	...
	public static int getProjectileShowDelay(Player c, boolean weapon) {
		...
		Optional<Integer> original = PoisonedWeapon.getOriginal(itemId);
		int ammo = original.orElse(itemId);
		int[] data = { 806, 806, 808, 809, 810, 811, 10033, 10034, 11959, 11230, 25849, };
		int str = 53;
	...
	public static boolean wearingCrossbow(Player player) {
		return player.playerEquipment[Player.playerWeapon] == 11785 || player.playerEquipment[Player.playerWeapon] == 9185 || player.playerEquipment[player.playerWeapon] == 23145
				|| player.playerEquipment[Player.playerWeapon] == 21012  || player.playerEquipment[Player.playerWeapon] == 24119 || player.playerEquipment[Player.playerWeapon] == 26374 || player.playerEquipment[Player.playerWeapon] == 21902;
	}
