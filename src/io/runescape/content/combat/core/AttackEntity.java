...
import io.runescape.content.bosses.hydra.HydraStage;
...
import io.runescape.content.combat.range.Bow;
import io.runescape.content.combat.range.RangeData;
import io.runescape.content.combat.specials.Special;
import io.runescape.content.combat.specials.Specials;
import io.runescape.content.combat.weapon.AttackStyle;
import io.runescape.content.combat.weapon.RangedWeaponType;
...
import io.runescape.content.skills.herblore.PoisonedWeapon;
...
import io.runescape.model.collisionmap.PathChecker;
...
import io.runescape.model.entity.npc.NPCClipping;
...
public class AttackEntity {
    ...
    public int getAttackDelay() {
        ...
        if (attacker.playerAttackingIndex > 0 && attacker.playerEquipment[Player.playerWeapon] == 12926  || attacker.playerEquipment[Player.playerWeapon] == 24106) {
    ...
    public void attackEntity(Entity targetEntity) {
        ...
        if (Boundary.isIn(attacker, Boundary.Level_54_WILD) && !EventBossHandler.participants.contains(attacker)) {//Glodd boss
            EventBossHandler.participants.add(attacker);
            return;
        }

        if (Boundary.isIn(attacker, Boundary.SOLAK_AREA) && !SolakEventBossHandler.participants.contains(attacker)) {//solak boss
            SolakEventBossHandler.participants.add(attacker);
            return;
        }

        if (Boundary.isIn(attacker, Boundary.KORANIAN_AREA) && !KoranianEventBossHandler.participants.contains(attacker)) {//koranian boss
            KoranianEventBossHandler.participants.add(attacker);
            return;
        }
        ...
        if (targetEntity.isNPC() && !attacker.usingClickCast) {
            ...
            if (attacker.playerEquipment[Player.playerWeapon] == 25731) {//Holy sang staff
                attacker.usingMagic = true;
                attacker.autocasting = true;
                attacker.setSpellId(SanguinestiStaff.COMBAT_SPELL_INDEX);
            }
        ...
        if (attacker.usingSpecial
            ...
            if (getCombatType() == CombatType.MAGE) {
                ...
                if (getCombatType() == CombatType.RANGE) {
                    ...
                    if (attacker.playerEquipment[Player.playerWeapon] != 24106)
                        attacker.gfx100(RangeData.getRangeStartGFX(attacker));
                    fireRangeProjectile(targetEntity);
    ...
    private void handleItemChangesOnAttack(Entity targetEntity) {
        ...
        if (!attacker.usingMagic) {
            ...
            if (targetEntity.isNPC()) {
                ...
                } else if (attacker.playerEquipment[Player.playerWeapon] == Items.SANGUINESTI_STAFF || attacker.playerEquipment[Player.playerWeapon] == 25731) {
    ...
    public void fireRangeProjectile(Entity entity) {
        ...
        int offX = (pY - nY) * -1;
        int offY = (pX - nX) * -1;
        if (attacker.weaponUsedOnAttack == 12926 || attacker.weaponUsedOnAttack == 24106) {
    ...
    private boolean determineWeaponStyle() {
        ...
        if (!attacker.usingMagic) {
            ...
            attacker.usingCross =
                    attacker.playerEquipment[Player.playerWeapon] == 4734 ||   attacker.playerEquipment[Player.playerWeapon] == 23145 ||attacker.playerEquipment[Player.playerWeapon] == 9185 || attacker.playerEquipment[Player.playerWeapon] == 11785 || attacker.playerEquipment[Player.playerWeapon] == 21012 || attacker.playerEquipment[Player.playerWeapon] == 24119 || attacker.playerEquipment[Player.playerWeapon] == 26374|| attacker.playerEquipment[Player.playerWeapon] == 21902;
            ...
            if (attacker.getItems().isWearingItem(24106)) {
                if (attacker.getToxicBlowpipeAmmo() == 0 || attacker.getToxicBlowpipeAmmoAmount() == 0 || attacker.getToxicBlowpipeCharge() == 0) {
                    attacker.sendMessage("Your vorkath blowpipe is out of ammo or charge.");
                    attacker.attacking.reset();
                    return false;
                }
                attacker.usingBow = true;
            ...
            if (!attacker.usingBallista && !attacker.usingCross && !attacker.usingArrows && attacker.usingBow && (((attacker.playerEquipment[Player.playerWeapon] < 4212)
                    && !(attacker.playerEquipment[Player.playerWeapon] == 20999) //Boffa normal
                    && !(attacker.playerEquipment[Player.playerWeapon] == 25867)))//boffa c
            ...
            if (!Bow.canUseArrow(attacker) && Configuration.CORRECT_ARROWS && attacker.usingBow && !attacker.getCombatItems().usingCrystalBow() && !(attacker.playerEquipment[Player.playerWeapon] == 22550) && attacker.playerEquipment[Player.playerWeapon] != 9185
                    && attacker.playerEquipment[Player.playerWeapon] != 4734 && attacker.playerEquipment[Player.playerWeapon] != 11785 && attacker.playerEquipment[Player.playerWeapon] != 21012 && attacker.playerEquipment[Player.playerWeapon] != 24119 && attacker.playerEquipment[Player.playerWeapon] != 26374  && attacker.playerEquipment[Player.playerWeapon] != 23145 && attacker.playerEquipment[Player.playerWeapon] != 12926 && attacker.playerEquipment[Player.playerWeapon] != 24106 && attacker.playerEquipment[Player.playerWeapon] != 19478 && attacker.playerEquipment[Player.playerWeapon] != 19481 && attacker.playerEquipment[Player.playerWeapon] != 21902) {
            ...
            if (attacker.playerEquipment[Player.playerWeapon] == 24111 && !attacker.usingArrows && attacker.usingBow) {// zilly bow
                attacker.sendMessage("You have run out of arrows!");
                reset();
                return false;
            }

            if (attacker.playerEquipment[Player.playerWeapon] == 20997 && !attacker.usingArrows && attacker.usingBow) {// twisted bow
                attacker.sendMessage("You have run out of arrows!");
                reset();
                return false;
            }

            if (attacker.playerEquipment[Player.playerWeapon] == 11235 && !attacker.usingArrows && attacker.usingBow) {// dark bow
                attacker.sendMessage("You have run out of arrows!");
                reset();
                return false;
            }

            if (attacker.playerEquipment[Player.playerWeapon] == 12768 && !attacker.usingArrows && attacker.usingBow) {// dark bow
                attacker.sendMessage("You have run out of arrows!");
                reset();
                return false;
            }

            if (attacker.playerEquipment[Player.playerWeapon] == 12424 && !attacker.usingArrows && attacker.usingBow) {// 3rd age bow
                attacker.sendMessage("You have run out of arrows!");
                reset();
                return false;
            }

            if (attacker.playerEquipment[Player.playerWeapon] == 12788 && !attacker.usingArrows && attacker.usingBow) {// mage short bow i
                attacker.sendMessage("You have run out of arrows!");
                reset();
                return false;
            }
            ...
            if (attacker.playerEquipment[Player.playerWeapon] == 24119 && !attacker.getCombatItems().properBolts() || attacker.playerEquipment[Player.playerWeapon] == 23145 && !attacker.getCombatItems().properBolts() || attacker.playerEquipment[Player.playerWeapon] == 26374 && !attacker.getCombatItems().properBolts()) {
                attacker.sendMessage("You must use bolts with a crossbow.");
                return false;
            }
    ...
    public static int getRangeDistance(Player player) {
        ...
        if (name.contains("twisted bow"))
            ...
            return 10;
        else if (name.contains("zilyana's godbow"))
            return 10;
    ...
    public boolean checkNpcAttackDistance(NPC npc, Player attacker) {
        ...
        if (getCombatType() != CombatType.MELEE) {
            ...
            if (npc.getNpcId() == 1101) {//powerful sea snake
                attackDistanceRequired = 50;
            }
