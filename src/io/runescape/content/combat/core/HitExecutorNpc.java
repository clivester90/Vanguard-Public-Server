...
import io.runescape.content.bosses.Tekton;
...
public class HitExecutorNpc extends HitExecutor {
    ...
    public void onHit() {
        ...
        Nex.takeDamage(attacker, damage.getAmount());
       // AlchemicalHydra.negateDamage(attacker, npc, damage);
        ...
        switch (npc.getNpcId()) {
            ...
            case 5129: //Glod
                ...
                EventBossHandler.glodSpecial(attacker);
                break;

            case 3407: //Solak
                attacker.setSolakDamageCounter(attacker.getSolakDamageCounter() + damage.getAmount());
                break;

            case 3833: //koranian
                attacker.setKoraneDamageCounter(attacker.getKoraneDamageCounter() + damage.getAmount());
                break;

            case 11246: //revenant maledictus
                attacker.setRevDamageCounter(attacker.getRevDamageCounter() + damage.getAmount());
                break;

            case 11278: //nex
                if (!Boundary.isIn(attacker, Boundary.NEX_BOSS_ROOM)) {
                    attacker.attacking.reset();
                    attacker.sendMessage("You cannot do this from here.");
                    return;
                }
