...
import io.runescape.content.combat.effects.damageeffect.impl.ToxicBlowpipeEffect;
import io.runescape.content.combat.effects.damageeffect.impl.ToxicStaffOfTheDeadEffect;
import io.runescape.content.combat.effects.damageeffect.impl.amuletofthedamned.impl.GuthanEffect;
import io.runescape.content.combat.effects.special.impl.ScytheOfVitur;
...
public abstract class HitDispatcher {
    ...
    private void playerHitEntity(CombatType combatType, Special special, boolean applyingMultiHitAttack) {
        ...
        if (combatType.equals(CombatType.MELEE)) {
            ...
            //if (attacker.isPrintAttackStats() && !applyingMultiHitAttack) {
                if (Configuration.DEBUG_MODE && !applyingMultiHitAttack) {
           ...
           // boolean hasDarkHealerVersion = attacker.petSummonId == 30118 || attacker.petSummonId == 30122;
            boolean hasDarkHealerVersion = attacker.getDamageTakenPerk() || attacker.petSummonId == 30122;
         ...
         ////       attacker.getPA().refreshSkill(5);
          //  }
            ...
            boolean hasDarkPrayerVersion = attacker.getHalfHitPerk();
            int prayerChance = hasDarkPrayerVersion ? 10 : 20;
            if (damage > 0 && attacker.hasFollower && ((attacker.petSummonId == 30019 || attacker.petSummonId == 30022) || hasDarkPrayerVersion) && Misc.random(prayerChance) == 1) {
                int halfDamage = (int) (damage / 2);
                attacker.playerLevel[5] += (halfDamage);
    ...
    private void dropArrows() {
        ...
        if (attacker.playerEquipment[Player.playerWeapon] == 24106 ) {//vorkath blowpipe
            return;
        }
        dropArrow(itemId);
    ...
    private void dropArrow(int arrowId) { // TODO delete arrows from player
        ...
        if (attacker.playerEquipment[Player.playerWeapon] == 24106) {// vorkath blow pipe
            return;
        }
        ...
        if (Misc.trueRand(3) == 0) {
            if(Boundary.isIn(attacker, Boundary.SEA_SNAKE_DECK)) {
                Server.itemHandler.createGroundItem(attacker, arrowId, attacker.getX(), attacker.getY(), height, 1, attacker.getIndex());
                return;
            }
