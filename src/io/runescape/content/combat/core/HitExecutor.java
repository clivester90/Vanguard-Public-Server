...
import io.runescape.content.combat.effects.damageeffect.impl.TridentOfTheSwampEffect;
    ...
    public void hit() {
        ...
        if (damage.getCombatType() != null) {
            ...
            switch (damage.getCombatType()) {
                case MELEE:
                    if (attacker.getItems().isWearingItem(24780, Player.playerAmulet) && Misc.hasOneOutOf(5)) {//Amulet of blood fury

                        if (attacker.hasFollower && (attacker.petSummonId == 30122)) {//Dark kratos
                            return;
                        }
                        if (attacker.hasFollower && (attacker.petSummonId == 30022)) {//regular kratos
                            return;
                        }
                        if (attacker.hasFollower && (attacker.petSummonId == 30018)) {//healer death spawn
                            return;
                        }
                        if (attacker.hasFollower && (attacker.petSummonId == 30118)) {//dark healer death spawn
                            return;
                        }
                        attacker.getHealth().increase(damage.getAmount() / 2);
                    }

                    if (attacker.hasFollower && attacker.petSummonId == 16012 && Misc.hasOneOutOf(25)) {//Revenant maledictus
                        attacker.getHealth().increase(damage.getAmount() / 2);
                        attacker.sendMessage("@blu@The pet manages to restore some of your health!");
                    }
                ...
                case MAGE:
                    ...
                    if (damage.isSuccess()) {
                        ...
                        if (attacker.playerEquipment[Player.playerWeapon] == 22323) {// sang staff
                                if (Misc.random(6) == 0) {//Sang special effect
                                    attacker.getHealth().increase(damage.getAmount() / 2);
                                    attacker.gfx0(1542);
                                 }
                        }
