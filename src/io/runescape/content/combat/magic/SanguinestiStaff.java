package io.runescape.content.combat.magic;
...
public class SanguinestiStaff {
    ...
    public static boolean useItem(Player player, int item1, int item2) {
        ...
        if (item1 == Items.BLOOD_RUNE && item2 == 25731 || item2 == Items.BLOOD_RUNE && item1 == 25731) {//Holy sang staff
            charge(player);
            return true;
        }
    ...
    public static boolean clickItem(Player player, int itemId, int option) {
        if (itemId == Items.SANGUINESTI_STAFF || itemId == 25731) {//Holy sang staff
            if (option == 5) {
                uncharge(player);
            } else if (option == 3) {
                charge(player);
    ...
    private static void charge(Player player) {
        if (player.getItems().playerHasItem(Items.SANGUINESTI_STAFF) || player.getItems().playerHasItem(25731)) {//Holy sang staff
