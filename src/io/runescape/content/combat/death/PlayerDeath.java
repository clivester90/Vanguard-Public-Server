package io.runescape.content.combat.death;
...
import io.runescape.content.combat.pvp.PkpRewards;
import io.runescape.content.itemskeptondeath.ItemsLostOnDeath;
import io.runescape.content.itemskeptondeath.ItemsLostOnDeathList;
...
import io.runescape.util.logging.player.DeathItemsHeld;
import io.runescape.util.logging.player.DeathItemsKept;
import io.runescape.util.logging.player.DeathItemsLost;
import io.runescape.util.logging.player.DeathLog;
...
public class PlayerDeath {
    ...
    public static void applyDead(Player c) {
        ...
        if (c.getRights().isOrInherits(Right.EVENT_MAN)) {
            if (Boundary.isIn(c, Boundary.DUEL_ARENA) || Boundary.isIn(c, Boundary.FIGHT_CAVE)
                    ...
                    || Boundary.isIn(c, Boundary.LMS_AREAS)
            ...
            PlayerHandler.executeGlobalMessage("@red@News: @blu@" + c.getDisplayNameFormatted()
                    ...
                    + "! ");
            Discord.writeDeathMessage("News: "+ c.getLoginName() + " has just died, with a skill total of " + c.totalLevel
                    + "! ");
        }
            ...
            if (!Configuration.DISABLE_HC_LOSS_ON_DEATH) {
                ...
                if (c.totalLevel > 500) {
                    PlayerHandler.executeGlobalMessage("@red@News: @blu@" + c.getDisplayNameFormatted()
                            ...
                            + "!");
                    Discord.writeDeathMessage("News: "+ c.getLoginName() + " has just died in hardcore ironman mode, with a skill total of " + c.totalLevel
                            + "!");
                }
        ...
        if (Objects.isNull(duelSession)) {
            ...
            if (killer != null) {
                ...
                if (killer instanceof Player) {
                    ...
                    if (killer != null) {//toplist
                        Feedlist.submit("@whi@[@gre@" + Misc.getCurrentServerTime() + "@whi@] " + "@or1@" + ((Player) killer).getLoginName()
                                + " @whi@killed @yel@" + c.getLoginName() + "@whi@!");
                        AchievementHandler.activate(((Player) killer), AchievementList.PVP_I, 1);//NEW ACHIEVEMNTS
                        AchievementHandler.activate(((Player) killer), AchievementList.PVP_II, 1);//NEW ACHIEVEMNTS
                        AchievementHandler.activate(((Player) killer), AchievementList.PVP_III, 1);//NEW ACHIEVEMNTS
                    }
                }
        ...
        if (c.prayerActive[CombatPrayer.RETRIBUTION]) {
            c.gfx0(437);
    ...
    private static void handleAreaBasedDeath(Player c) {
        ...
        // Unsafe death
        if (c.getPosition().inWild() || Boundary.isIn(c, Boundary.DEATH_NPC_AREA)) {
        ...
        } else if (Boundary.isIn(c, Boundary.KRATOS_AREA)) {//Kratos
            NPCHandler.despawn(6009, 0);
            NPCHandler.despawn(6008, 0);
            KratosNpc.spawned = false;



        } else if (Boundary.isIn(c, Boundary.NEX_BOSS_ROOM)) {//nex
            c.nexDeaths+=1;
        ...
        if (Boundary.isIn(c, Boundary.CERBERUS_BOSSROOMS)) {
            ...
            if (Boundary.isIn(c, Boundary.DEATH_NPC_AREA)) {//Death npc for ultimate ironmen
               c.getPA().movePlayer(new Coordinate(3084, 3515));
           } else {
                c.getPA().movePlayer(Configuration.RESPAWN_X, Configuration.RESPAWN_Y, 0);
            }
