package io.runescape.content.combat.formula;
...
public class MeleeMaxHit {
	...
	public static int calculateBaseDamage(Player c) {
		...
		if (c.npcAttackingIndex > 0) {
			...
			if (npc != null) {
				if (c.getSlayer().hasSlayerHelmBoost(npc, CombatType.MELEE)) {
					...
					if (c.getItems().isWearingItem(12018, Player.playerAmulet)) {//Salve amulet ei
						if (Misc.linearSearch(Configuration.UNDEAD_NPCS, npc.getNpcId()) != -1) {
							base *= 1.20;
						}
					} else if (c.getItems().isWearingItem(10588, Player.playerAmulet)) {// Salve amulet e
						if (Misc.linearSearch(Configuration.UNDEAD_NPCS, npc.getNpcId()) != -1) {
							base *= 1.20;
						}
					} else if (c.getItems().isWearingItem(4081, Player.playerAmulet)) {// Salve amulet
						if (Misc.linearSearch(Configuration.UNDEAD_NPCS, npc.getNpcId()) != -1) {
							base *= 1.15;
						}
				...
				if (c.getItems().isWearingItem(24158)) {//krill swords
					//int maxHit = attacker.getCombat().calculateMeleeMaxHit();
					int maxHit = 70;
					if (base > 0 && Misc.random(1) == 0)
						base = Misc.random(maxHit) / 2;
				}


				if (c.getItems().isWearingItem(22557)) {//amulet of averice
					NPC npc1 = NPCHandler.npcs[c.npcAttackingIndex];
					if (Misc.linearSearch(Configuration.REV_IDS, npc.getNpcId()) != -1) {
						base *= 1.80;
					}
				}
				...
				if (c.getSlayer().getTask().isPresent() && c.getSlayer().getTask().get().getPrimaryName().contains("crystalline") && c.getItems().playerHasItem(26484)) {
					base *= 2.20;
				}
		...
		boolean hasDarkVersion = (c.getMeleeStrengthPerk() || c.petSummonId == 30120 || c.petSummonId == 30122);
		if (c.hasFollower && ((
	...
	public static int calculateMeleeAttack(Player c) {
		...
		if (c.playerEquipment[Player.playerWeapon] == 25736) {//holy scythe of vitur
			attackLevel += c.getLevelForXP(c.playerXP[Player.playerAttack]) * 1.4;
		}
