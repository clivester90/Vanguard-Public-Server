...
public class MagicMaxHit {
		...
		if (c.npcAttackingIndex > 0 && c.getSlayer().getTask().isPresent()) {
			...
			if (npc != null) {
				...
				// Slayer helmet
					...
					modifier += .15;
			    } else if (c.getItems().isWearingItem(22557, Player.playerAmulet) && Misc.linearSearch(Configuration.REV_IDS, npc.getNpcId()) != -1) {//amulet of averice
					modifier += .20;
				}
			}
		}
	...
	//	boolean hasDarkVersion = (c.petSummonId == 30117 || c.petSummonId == 30120 || c.petSummonId == 30122);
		boolean hasDarkVersion = (c.getMagicStrengthPerk() || c.petSummonId == 30120 || c.petSummonId == 30122);
