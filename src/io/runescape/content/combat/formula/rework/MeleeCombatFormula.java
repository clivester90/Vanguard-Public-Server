package io.runescape.content.combat.formula.rework;

import io.runescape.content.combat.CombatConfigs;
...
import io.runescape.content.commands.owner.SetAccuracyBonus;
import io.runescape.content.commands.owner.SetDefenceBonus;
...
public class MeleeCombatFormula implements CombatFormula {
    ...
    public int applyExtras(double maxHit, Entity attacker, Entity defender) {
        if (attacker.isPlayer()) {
           ...
           // if (attacker.isPlayer() && weapon == 26484) {//tentacle (or)
           //     maxHit *= 0.45;
            //}
    ...
    private double getNpcMultipliers(Player attacker, NPC defender) {
        ...
        if (attacker.getItems().isWearingItem(26484, Player.playerWeapon))//tentacle (or)
            multiplier += 0.55;
        ...
        if (attacker.getItems().isWearingItem(25734, Player.playerWeapon)) {// holy grazi rapier
             multiplier += 0.4;
            if (Misc.random(1, 5) == 1) {
                attacker.sendMessage("@blu@Saradomin has blessed your attack with more damage.");
                multiplier += 0.7;
            }
        }
