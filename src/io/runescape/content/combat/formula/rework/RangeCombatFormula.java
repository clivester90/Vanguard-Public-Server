...
public class RangeCombatFormula implements CombatFormula {
    ...
    private double getNpcAttackMultiplier(Player attacker, NPC defender) {
        ...
        if (attacker.getItems().isWearingItem(24119, Player.playerWeapon))//black dhcb
            if (defender.isDragon())
                multiplier += 0.3;

        if (attacker.getItems().isWearingItem(26374, Player.playerWeapon))//zaryte bow
            if (defender.isDragon())
                multiplier += 0.7;
    ...
    public int getMaxHit(Entity attacker, Entity defender, double specialAttackMultiplier,
        ...
        if (maxhit != -1) {
            ...
            if (attacker.isPlayer()) {
                ...
                if (player.getItems().isWearingItem(Items.TOXIC_BLOWPIPE) || player.getItems().isWearingItem(24106, Player.playerWeapon)) {
    ...
    private double getEffectiveRangeLevel(Player player, Entity target) {
        ...
        if (player.fullCrystal()) {
            effectiveLevel *= 1.25;
            effectiveLevel = Math.floor(effectiveLevel);
        }
        if (player.fullCrystalArmourC()) {
            effectiveLevel *= 1.35;
            effectiveLevel = Math.floor(effectiveLevel);
        }
    ...
    private double getNpcMultipliers(Player attacker, NPC defender) {
        ...
        if (attacker.getItems().isWearingItem(26374, Player.playerWeapon))//zartye bow
            if (defender.isDragon())
                multiplier += 0.65;
        ...
        if (attacker.getItems().isWearingItem(24111, Player.playerWeapon)) {// zilly bow
            multiplier += 0.2;
            if (Misc.random(1, 5) == 1) {
                attacker.sendMessage("@blu@The power from Zilyana's Godbow strikes your opponent with additional damage..");
                multiplier += 0.4;
            }
        }

        if ((attacker.fullCrystal())) {//full crystal with boffa
            multiplier += 0.45;
        }
        if ((attacker.fullCrystalArmourC())) {//full crystal (c) with boffa (c)
            multiplier += 0.75;
        }
