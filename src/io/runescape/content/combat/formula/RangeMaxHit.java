...
public class RangeMaxHit extends RangeData {
	...
	public static int calculateRangeAttack(Player c) {
		...
		if (c.npcAttackingIndex > 0 && c.getItems().isWearingItem(23145)) {//Twisted crossbow
			Optional<NPC> npc = NPCHandler.getNpcAtIndex(c.npcAttackingIndex);
			if (npc.isPresent()) {
				double boost = getTwistedBowAccuracyBoost(getBestTwistedBowMagicLevel(npc.get()));
				rangeLevel += (rangeLevel * boost);
				if (c.debugMessage) {
					c.sendMessage("Twisted crossbow accuracy boost: " + boost);
				}
			}
		}
		...
		if (c.playerEquipment[c.playerWeapon] == 24106) {// vorkathblowpipe
			rangeLevel *= .95;
		}

		if (c.playerEquipment[c.playerWeapon] == 24111) {// zilly bow
			rangeLevel *= 3.25;
		}
	...
	public static int maxHit(Player c) {
		...
		} else if (c.playerEquipment[Player.playerWeapon] == Items.TOXIC_BLOWPIPE || c.playerEquipment[Player.playerWeapon] == 24106 ) {//blowpipe
		...
		if (c.npcAttackingIndex > 0) {
			...
			if (c.getItems().isWearingItem(21012) || c.getItems().isWearingItem(24119) || c.getItems().isWearingItem(26374) || c.getItems().isWearingItem(23145)) {
			...
			} else if (c.getItems().isWearingItem(22557, Player.playerAmulet)) {// AMULET OF AVERACE
	...
	//		if (hasDarkVersion) {
			...
			boolean hasDarkVersion = (c.getRangeStrengthPerk());
		...
		if (c.npcAttackingIndex > 0) {
			...
			if (c.getItems().isWearingItem(Items.TWISTED_BOW) || c.getItems().isWearingItem(23145)) {
