package io.runescape.content.skills.runecrafting;
...
public class Runecrafting extends SkillHandler {
	...
	public enum RunecraftingData {
		...
		BLOOD(43848, 565, 17, new int[] { 77, 89, 94 }, 18800, 20691),
		...
		for (RunecraftingData data : RunecraftingData.values()) {
			...
			if (data.getObjectId() == objectId) {
				...
				if (name.equals("nature")) {
					...
					DailyTasks.increase(player, DailyTasks.PossibleTasks.NATURE_RUNES);//Daily tasks
				}

				if (name.equals("law"))
					DailyTasks.increase(player, DailyTasks.PossibleTasks.LAW_RUNES);//Daily tasks


				for (int item1 : Configuration.RAIMENTS_OF_THE_EYE_PEICES)
				if (player.getItems().isWearingItem(item1)) {//Raiments of the Eye all colours piece bonus
					multiply *= 1.30;// 30% more runes
					//player.sendMessage("@pur@Raiments of the eye grants you an additional 30% per piece of raiments.");
				}
				//if (player.Raiments_of_the_Eye_Set()) {
				//	multiply *= 1.60;//60%
				//	player.sendMessage("@pur@Raiments of the eye grants you an additional 60%.");
				//} else if (player.Raiments_of_the_Eye_Set_Green()) {
				//	multiply += 1.60;//60%
				//	player.sendMessage("@pur@Raiments of the eye grants you an additional 60%.");
				//} else if (player.Raiments_of_the_Eye_Set_Blue()) {
				//	multiply += 1.60;//60%
				//	player.sendMessage("@pur@Raiments of the eye grants you an additional 60%.");
				//} else if (player.Raiments_of_the_Eye_Set_Red()) {
				//	multiply += 1.60;//60%
				//	player.sendMessage("@pur@Raiments of the eye grants you an additional 60%.");
				//}
				...
				player.getPA().addSkillXP((int) (((data.getExperience()) * count) + (player.getItems().isWearingItem(20008) ? percentOfXp : 0)), 20, true);
				player.getItems().addItem(data.getRuneId(), multiply);
				player.getPA().requestUpdates();
				player.getPA().sendSound(2710);
				petRoll(player, data);
				if (Misc.hasOneOutOf(25)) {//Runecrating casket
