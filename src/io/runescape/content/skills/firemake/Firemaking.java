package io.runescape.content.skills.firemake;
	...
	public static void lightFire(final Player player, final int logUsed, final String usage) {
		...
		if (log.getlogId() == logUsed) {
			...
			if (usage == "tinderbox") {
				...
				if (log.getlogId() == 1511) {//normal logs
					AchievementHandler.activate(player, AchievementList.BURN_200_LOGS, 1);//NEW ACHIEVEMNTS
				}
				...
				if (log.getlogId() == 1515)//Daily tasks
					DailyTasks.increase(player, DailyTasks.PossibleTasks.LIGHT_YEW_LOGS);
				if (log.getlogId() == 1513)//Daily tasks
					DailyTasks.increase(player, DailyTasks.PossibleTasks.LIGHT_MAGIC_LOGS);
				CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
					@Override
					public void execute(CycleEventContainer container) {
			...
			if (Misc.hasOneOutOf(25)) {//Firemaking tomes
				player.getItems().addItemUnderAnyCircumstance(7794, 1);
				player.sendMessage("@red@You manage to receive an firemaking tome.");
			}
