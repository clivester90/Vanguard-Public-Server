...
import io.runescape.content.bosses.Kraken;
			...
			if (isTaskNpc(npc)) {
				...
				master.ifPresent(m -> {
					switch (m.getId()) {
						case 8623:
							if (KonarSlayer.checkLocation(player)) {
								...
								if (npc.getNpcId() == 494) {
									player.getPA().addSkillXP(255, Skill.SLAYER.getId(), true);
								} else {
									player.getPA().addSkillXP(player.getRechargeItems().hasAnyItem(13113, 13114, 13115) && Boundary.isIn(player, Boundary.SLAYER_TOWER_BOUNDARY) ? (int) (task.getExperience() * 1.10) : task.getExperience(),
					...
					if (taskAmount == 0) {
						...
						if (Misc.hasOneOutOf(8)) {//Slayer tomes
							player.getItems().addItemUnderAnyCircumstance(7788, 1);
							player.sendMessage("@red@You manage to receive an slayer tome.");
						}
	...
	public void handleSuperiorExp(NPC npc) {
		task.ifPresent(task -> {
			...
			if (!name.equals(task.getPrimaryName())) {
				...
				if (isSuperiorNpc()) {
					player.getPA().addSkillXP(player.getRechargeItems().hasAnyItem(13113, 13114, 13115) && Boundary.isIn(player, Boundary.SLAYER_TOWER_BOUNDARY) ? (int) (task.getExperience() * 1.10)
