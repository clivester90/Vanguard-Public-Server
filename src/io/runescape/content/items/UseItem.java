...
import io.runescape.content.bosses.bryophyta.BryophytaNPC;
...
import io.runescape.content.item.lootable.impl.CrystalChest;
import io.runescape.content.item.lootable.impl.FoeMysteryBox;
import io.runescape.content.items.item_combinations.Godswords;
import io.runescape.content.items.pouch.RunePouch;
import io.runescape.content.lootbag.LootingBag;
import io.runescape.content.minigames.warriors_guild.AnimatedArmour;
...
import io.runescape.content.skills.firemake.Firemaking;
import io.runescape.content.skills.herblore.*;
import io.runescape.content.skills.prayer.Bone;
import io.runescape.content.skills.prayer.Prayer;
import io.runescape.content.skills.slayer.SlayerUnlock;
import io.runescape.content.trails.MasterClue;
...
import io.runescape.model.entity.player.packets.objectoptions.impl.DarkAltar;
import io.runescape.model.items.GameItem;
import io.runescape.model.items.ItemCombination;
import io.runescape.util.Misc;
import io.runescape.util.logging.player.ItemOnItemLog;
...
public class UseItem {
	...
	public static void ItemonObject(Player c, int objectID, int objectX, int objectY, int itemId) {
		...
		switch (objectID) {
			...
			case 41423://afk range
			case 27517://afk range
				for (final AfkRange.Food g : AfkRange.Food.values()) {
					if (itemId == g.getLogID()) {
						AfkRange.startCooking(c, itemId);
						return;
					}else {
						c.sendStatement("You cannot cook that item.");
					}
				}
			...
			case 10082://afk furnace
				for (final AfkFurnace.Bars g : AfkFurnace.Bars.values()) {
					if (itemId == g.getLogID()) {
						AfkFurnace.startMelting(c, itemId);
						return;
					}else {
						c.sendStatement("You cannot smelt that item.");
					}
				}
				c.facePosition(objectX, objectY);
				break;
			...
			case 28791://afk bonfire
				for (final Bonfire.Logs g : Bonfire.Logs.values()) {
					if (itemId == g.getLogID()) {
						Bonfire.startBurning(c, itemId);
						return;
					}else {
						c.sendStatement("You cannot burn that item.");
					}
				}
				c.facePosition(objectX, objectY);
				break;
		...
		//case 26782: // OLD SYSTEM FOR FOUNTAIN OF RUNE
	//		c.objectYOffset = 5;
	//		c.objectXOffset = 5;
	//		c.objectDistance = 5;
	//	    switch (itemId) {
	...
	public static void ItemonItem(final Player c, final int itemUsed, final int useWith, final int itemUsedSlot, final int usedWithSlot) {
		...
		if (itemUsed == 20002 && useWith == 4587) {//scimmi or
			c.getItems().deleteItem(itemUsed, 1);
			c.getItems().deleteItem(useWith, 1);
			c.getItems().addItem(20000, 1);
			c.getDH().sendStatement("You combined the items and create the @red@dragon scimitar(or)");
		}

		if (itemUsed == 25742 && useWith == 22325) {//holy scythe kit
			c.getItems().deleteItem(itemUsed, 1);
			c.getItems().deleteItem(useWith, 1);
			c.getItems().addItem(25736, 1);
			c.getDH().sendStatement("You combined the items and create the holy scythe of vitur.");
		}

		if (itemUsed == 25742 && useWith == 22324) {//holy GRAZI kit
			c.getItems().deleteItem(itemUsed, 1);
			c.getItems().deleteItem(useWith, 1);
			c.getItems().addItem(25734, 1);
			c.getDH().sendStatement("You combined the items and create the holy ghrazi rapier.");
		}

		if (itemUsed == 25742 && useWith == 22323) {//holy sang kit
			c.getItems().deleteItem(itemUsed, 1);
			c.getItems().deleteItem(useWith, 1);
			c.getItems().addItem(25731, 1);
			c.getDH().sendStatement("You combined the items and create the holy sanguinesti staff.");
		}


		if (itemUsed == 21752 && useWith == 6806) {//ring of koranes making
		...
		if (itemUsed == 26370 && useWith == 11798) {//ancient godsword making
			c.getItems().deleteItem(26370, 1);
			c.getItems().deleteItem(11798, 1);
			c.getItems().addItem(26233, 1);
			c.getDH().sendStatement("You combined the items and create an ancient godsword.");
		}

		if (itemUsed == 5075 && useWith == 233) {//Crush nests
			c.getItems().deleteItem(itemUsed, 1);
			c.getItems().addItem(6693, 1);//crushed nest
			c.getDH().sendItemStatement("You managed to crush the nest.", 6693);
		}

		if (itemUsed == 233 && useWith == 5075) {//Crush nests
			c.getItems().deleteItem(useWith, 1);
			c.getItems().addItem(6693, 1);//crushed nest
			c.getDH().sendItemStatement("You managed to crush the nest.", 6693);
		}

		if (itemUsed == 26372 && useWith == 11785) {//zartye cross bow making
			c.getItems().deleteItem(26372, 1);
			c.getItems().deleteItem(11785, 1);
			c.getItems().addItem(26374, 1);
			c.getDH().sendStatement("You combined the items and create an zaryte crossbow.");
		}
		...
		if (itemUsed == 21043 && useWith == 6914 || (itemUsed == 6914 && useWith == 21043)) {
			if (c.getItems().playerHasItem(21043) &&
				...
				c.getDH().sendItemStatement("You successfully make the kodai wand", 21006);
			}
		}
		if (itemUsed == 22477 && useWith == 12954 || itemUsed == 12954 && useWith == 22477) {
			...
			c.getDH().sendItemStatement("You successfully make the avernic defender", 22322);
		}
		if (itemUsed == 2425 && useWith == 10499) {
			c.getItems().deleteItem(2425, 1);
			c.getItems().deleteItem(10499, 1);
			c.getItems().addItem(22109, 1);
			c.getDH().sendItemStatement("You successfully make the Ava's Assembler", 22109);
		}

		if (itemUsed == 2890 && useWith == 21637) {//Wyvern visage message
			c.sendMessage("Perhaps some of the magical apparatus on Fossil Island can help join these two items.");
		}


		if (itemUsed == 22006 && useWith == 1540) {
			if (c.getPA().getLevelForXP(c.playerXP[Skill.SMITHING.getId()]) < 90) {
				c.sendMessage("You need a smithing level of 90 to do this.");
				return;
			}
			c.getItems().deleteItem(itemUsed, 1);
			c.getItems().deleteItem(useWith, 1);
			c.getItems().addItem(22002, 1);
			c.getDH().sendItemStatement("You successfully make the Dragonfire Ward", 22002);
		}
		...
		if (itemUsed == 21295 && useWith == 13280 || itemUsed == 13280 && useWith == 21295) {
			if (!c.getItems().playerHasItem(13281)) {
				...
				c.getDH().sendItemStatement("You successfully make the Infernal Max Cape", 21285);
		}
		...
		if (itemUsed == 24107 || useWith == 24107) {//vork bp
			int ammo = itemUsed == 24107 ? useWith : itemUsed;
			ItemDef definition = ItemDef.forId(ammo);
			int amount = c.getItems().getItemAmount(ammo);
			if (ammo == 12934) {
				c.sendMessage("Select a dart to store and have the equivellent amount of scales.");
			...
			if (!c.getItems().playerHasItem(24107)) {
				c.sendMessage("You need a vorkath blowpipe (empty) to do this.");
				return;
			}
			if (amount > 16383) {
				c.sendMessage("The blowpipe can only store 16,383 charges at any given time.");
			...
			c.getItems().deleteItem2(24107, 1);
			c.getItems().addItem(24106, 1);
			c.getItems().deleteItem2(ammo, amount);
			c.getItems().deleteItem2(12934, amount);
			c.setToxicBlowpipeCharge(amount);
			c.setToxicBlowpipeAmmo(ammo);
			c.setToxicBlowpipeAmmoAmount(amount);
			c.sendMessage("You store " + amount + " " + definition.getName() + " into the vorkath blowpipe and charge it with scales.");
			return;
		}
		...
		if (itemUsed == 269 || useWith == 12695 || useWith == 11722 || useWith == 11726) {
			if (c.getLevelForXP(c.playerXP[Player.playerHerblore]) < 50) {
				c.sendMessage("You need a Herblore level of " + 50 + " to make an overload.");
				return;
			}
			if (c.getItems().playerHasItem(269) && c.getItems().playerHasItem(12695)
					&& c.getItems().playerHasItem(11722)
					&& c.getItems().playerHasItem(11726)) {
				c.getItems().deleteItem(269,1);
				c.getItems().deleteItem2(12695, 1);
