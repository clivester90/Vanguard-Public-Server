...
import static io.runescape.Server.npcHandler;
import static io.runescape.model.entity.player.PlayerHandler.getPlayers;
...
public class Nex {
	...
	public static void spawn() {
		...
		NPCHandler.despawn(11278, 0);//Nex
		despawn();

		NEX = NPC.of(11278, new Position(2924, 5203, 0));

		FUMUS = NPC.of(11283, new Position(2913, 5215, 0));
		UMBRA = NPC.of(11284, new Position(2937, 5215, 0));
		CRUOR = NPC.of(11285, new Position(2937, 5191, 0));
		GLACIES = NPC.of(11286, new Position(2913, 5191, 0));
	...
	public static void despawn() {
		...
		NPCHandler.despawn(11283, 0);

		NPCHandler.despawn(11284, 0);

		NPCHandler.despawn(11285, 0);

		NPCHandler.despawn(11286, 0);
	}
	...
	public void handleAttackTest(Player player) {
		...
		if (phase == 0) {
			...
			if (distance2 > 2) {
				npcHandler.followPlayer(nex, player.getIndex());
				nex.setAttackType(CombatType.MAGE);
				nex.hitDelayTimer = 2;
				nex.projectileId = 2009;//Smoke phase
				nex.endGfx = -1;
				nex.maxHit = 15;
		...
		if (phase == 3) {
			...
			if (distance2 > 2) {
				nex.setAttackType(CombatType.MAGE);
				nex.projectileId = 2004;
				nex.endGfx = -1;
				nex.maxHit = 20;
				if (player.prayerActive[16]) {
					return;
				} else {
					player.gfx0(2005);//ice barrage
					player.sendMessage("@red@Nex has frozen you to the ground!");
					...
					if (p.distanceToPoint(x, y) <= 20) {
						...
						for (int x_ = x - 2; x_ < x + 2; x_++) {
							for (int y_ = y - 2; y_ < y + 2; y_++) {
								p.getPA().createPlayersStillGfx(2013, x_, y_, 0, 5);//1247
							}
						}
					}
				}
	...
	private void walkToCenter(Player player) {
		...
		CycleEventHandler.getSingleton().addEvent(0, nex, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (cycle == 2) {
					nex.teleport(NORTH.getX(), NORTH.getY(), 0);
					nex.facePlayer(1);
