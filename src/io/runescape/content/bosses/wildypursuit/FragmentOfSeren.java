package io.runescape.content.bosses.wildypursuit;
...
import io.runescape.content.events.monsterhunt.MonsterHunt;
...
import io.runescape.model.entity.player.PathFinder;
...
public class FragmentOfSeren {
	...
	public static void handleSpecialAttack(Player player) {
		...
		NPC SEREN = NPCHandler.getNpc(8918);
		...
		if (SEREN.getHealth().getCurrentHealth() <= 600 && specialAmount == 0) {
			SEREN.forceChat("I will ruin you all!");
			SEREN.startAnimation(8380);
			SEREN.underAttackBy = -1;
			SEREN.underAttack = false;
			isAttackable = false;
			NPCHandler.queenAttack = "SPECIAL";
			specialAmount++;
			activePillars.clear();
			for(int i = 0; i < 3; i++) {
				if (!PathFinder.getPathFinder().accessable(player, SEREN.getX() + coords[i][0], SEREN.getY() + coords[i][1])) {
					activePillars.add(NPCSpawning.spawnNpcOld(CRYSTAL_WHIRLWIND, SEREN.getX(), SEREN.getY(), SEREN.getHeight(), 0, 200, 0, 0, 0));
				} else {
					activePillars.add(NPCSpawning.spawnNpcOld(CRYSTAL_WHIRLWIND, SEREN.getX() + coords[i][0], SEREN.getY() + coords[i][1], SEREN.getHeight(), 0, 200, 0, 0, 0));
				}
			}
