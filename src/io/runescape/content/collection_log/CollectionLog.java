package io.runescape.content.collection_log;
...
import io.runescape.content.item.lootable.impl.TheatreOfBloodChest;
import io.runescape.content.trails.RewardLevel;
import io.runescape.content.trails.TreasureTrailsRewardItem;
...
public class CollectionLog {
	...
	public static final int BARROWS_ID = 555;
			...
			for(int i = 0; i < npcs.size(); i++) {
				...
				if (getCollections().containsKey(npcs.get(i) + "")) {
					...
					if (itemsObtained != null) {
						...
						} else if (npcs.get(i) == BARROWS_ID) {
							drops = Barrows.getRareDrops();
						} else if (npcs.get(i) == 11278) {//Nex
							drops = Nex.getRareDrops();
						}
	...
	public void populateInterface(Player player, int npcId) {
		...
		if (npcId == BARROWS_ID) {
			player.dropItems = Barrows.getRareDrops();
			player.getPA().sendFrame126("Barrows runs: @whi@" + player.getBarrowsRunCompleted(), 23120);
		}
		if (npcId == 11278) {//Nex
			player.dropItems = Nex.getRareDrops();
			player.getPA().sendFrame126("Nex runs: @whi@" + player.nexKills, 23120);
		}
