...
import io.runescape.content.achievement.Achievements.Achievement;
...
import io.runescape.util.logging.player.ShopBuyLog;
import io.runescape.util.logging.player.ShopSellLog;



public class ShopAssistant {

	public final static double SELL_TO_SHOP_PRICE_MODIFIER = 0.6;//07
	...
	public void buyFromShopPrice(int removeId, int removeSlot) {
		...
		if (c.myShopId == ShatteredShardPrice.SHOP_ID) {
			String formattedPrice = Misc.formatCoins(ShatteredShardPrice.getBurnPrice(c, removeId, false));
			c.sendMessage("You can exchange {} for <col=255>{}</col> shards.", itemName, formattedPrice);
			return;
		}
		...
		if (c.myShopId == 189) {//trivia
			c.sendMessage(itemName + ": currently costs [ @pur@" + Misc.insertCommas(getSpecialItemValue(removeId)) + " @bla@] Trivia Points.");
			return;
		}

		if (c.myShopId == 205) {//daily task store
			c.sendMessage(itemName + ": currently costs [ @pur@" + Misc.insertCommas(getSpecialItemValue(removeId)) + " @bla@] Daily Task Points.");
			return;
		}

		if (c.myShopId == 199) {//afk
			c.sendMessage(itemName + ": currently costs [ @pur@" + Misc.insertCommas(getSpecialItemValue(removeId)) + " @bla@] Afk Points.");
			return;
		}
		if (c.myShopId == 201) {//new tokkul store
			c.sendMessage(itemName + ": currently costs [ @pur@" + Misc.insertCommas(getSpecialItemValue(removeId)) + " @bla@] Tokkul.");
			return;
		}
		...
		if (c.myShopId == 171) {
			c.sendMessage(itemName + "@bla@: currently costs [ @pur@" + getSpecialItemValue(removeId) + " @bla@] shards.");
			return;
		}
		...
		case 15:
			...
			case 9: //Donator Point Shop
				switch (id) {
					...
					case 21126://hazel mer signut
						return 85;
					case 962://christmas cracker
						return 40;
					case 6199://mystery box
						return 8;
					case 21752://ring of vanguard
						return 110;
					case 6806://vanguard imbued scroll
						return 100;
					case 21295://infernal cape
						return 50;
					case 8167://insane mysyery box
						return 90;
					case 6828://super mystery box
						return 15;
					case 13346://ultra mystery box
						return 50;
	...
	//	removeId = c.getItems().getUnnotedItem(removeId);
//		int itemPrice = getItemShopValue(removeId);
//		int value = itemPrice;
//		if (itemPrice > 1) {
...
//		}
		...
		if (!IsIn) {
			...
			if (c.myShopId == 206) {//High alch store
				//ShopValue = (int) (ShopAssistant.getItemShopValue(removeId) * .75);
				ShopValue *= 1.39;
			}
	...
	public boolean sellItem(int itemID, int fromSlot, int amount) {
		...
		if (Configuration.DISABLE_SS && c.myShopId == ShatteredShards.SS_SHOP_ID) {
		...
		if (amount > 0 && itemID == (c.playerItems[fromSlot] - 1)) {
			...
			if (c.myShopId == 206) {//high alch store
				//TotPrice2 = (int) (ShopAssistant.getItemShopValue(itemID) * .75);
				TotPrice2 *= 1.39;
			}
				...
				if ((ItemDef.forId(itemID).isStackable() || ItemDef.forId(itemID).isNoted()) && c.getItems().playerHasItem(itemID, amount)) {
					...
					if (c.myShopId != 12 && c.myShopId != 29 && c.myShopId != 44 && c.myShopId != 18  && c.myShopId != 83 && c.myShopId != 116 && c.myShopId != 115) {
						...
						logShop("received", 13307, (int) Math.ceil(TotPrice3 *= 0.30));
							...
							c.getItems().addItem(6466, (int) Math.ceil(TotPrice3 *= 0.30));
							logShop("received", 6466, (int) Math.ceil(TotPrice3 *= 0.30));
					...
					if (c.myShopId != 12 && c.myShopId != 29 && c.myShopId != 44 && c.myShopId != 18 && c.myShopId != 83 && c.myShopId != 116 && c.myShopId != 115) {
						...
						c.getItems().addItem(6466, (int) Math.ceil(TotPrice3 *= 0.60));
						logShop("received", 6466, (int) Math.ceil(TotPrice3 *= 0.60));
					}
					else if (c.myShopId == 44) {
						if (!ItemDef.forId(itemID).getName().contains("head")) {
							return false;
	...
	public void handleOtherShop(int itemID, int amount) {
		...
		if (c.myShopId == 189) {//Trivia store
			if (c.getItems().freeSlots() < 1) {
				c.sendMessage("You need at least one free slot to buy this.");
				return;
			}
			int itemValue = getSpecialItemValue(itemID) * amount;
			if (c.getTriviaPoints() < itemValue) {
		...
		if (c.myShopId == 205) {//daily task store
			...
			if (c.dailyTaskPoints < itemValue) {
				c.sendMessage("You do not have enough daily task points to buy this from the shop.");
				return;
			}
			c.dailyTaskPoints -= getSpecialItemValue(itemID) * amount;
			c.getItems().addItem(itemID, amount);
			c.getItems().sendInventoryInterface(3823);
			logShop("bought", itemID, amount);
			return;
		...
		if (c.myShopId == 199) {//Afk store
			if (c.getItems().freeSlots() < 1) {
				c.sendMessage("You need at least one free slot to buy this.");
				return;
			}
			int itemValue = getSpecialItemValue(itemID) * amount;
			if (c.afkPoints < itemValue) {
				c.sendMessage("You do not have enough afk points to buy this from the shop.");
				return;
			}
			c.afkPoints -= getSpecialItemValue(itemID) * amount;
			c.getItems().addItem(itemID, amount);
			c.getItems().sendInventoryInterface(3823);
			logShop("bought", itemID, amount);
			return;
		...
		if (c.myShopId == 201) {//tokkul store
			int itemValue = getSpecialItemValue(itemID) * amount;
			if (!c.getItems().playerHasItem(6529, itemValue)) {
				c.sendMessage("You do not have enough tokkul to purchase this.");
				return;
			}
			c.getItems().deleteItem(6529, itemValue);
			c.getItems().addItem(itemID, amount);
			c.getItems().sendInventoryInterface(3823);
			logShop("bought", itemID, amount);
			return;
		...
		if (c.myShopId == 197) {//lms store
			...
			int itemValue = getSpecialItemValue(itemID) * amount;
			if (c.getLastManPoints() < itemValue) {
				c.sendMessage("You do not have enough lms points to buy this from the shop.");
				return;
			}
			c.setLastManPoints(c.getLastManPoints() - itemValue);
			c.getItems().addItem(itemID, amount);
			c.getItems().sendInventoryInterface(3823);
			logShop("bought", itemID, amount);
			return;
		...
		if (c.myShopId == 171) {
			int itemValue = getSpecialItemValue(itemID) * amount;
			if (!c.getItems().playerHasItem(6466, itemValue)) {
				c.sendMessage("You do not have enough shards to purchase this.");
				return;
			}
			c.getItems().deleteItem(6466, itemValue);
			c.getItems().addItem(itemID, amount);
			c.getItems().sendInventoryInterface(3823);
			logShop("bought", itemID, amount);
			return;
		...
		if (c.myShopId == 44) {
			...
			if (c.exchangePoints >= getSpecialItemValue(itemID) * amount) {
				...
				c.sendMessage("You do not have enough shards to buy this item.");
			}
		} else if (c.myShopId == 172 || c.myShopId == 173) {
