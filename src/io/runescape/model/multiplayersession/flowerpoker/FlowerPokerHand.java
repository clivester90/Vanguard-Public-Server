...
import io.runescape.model.cycleevent.*;
...
import io.runescape.util.logging.player.FlowerpokerResultLog;
...
public class FlowerPokerHand {
    ...
    public static List<Position> positions = Arrays.asList(
            new Position(3112, 3488, 0),
            new Position(3112, 3491, 0),
            new Position(3112, 3494, 0),
            new Position(3112, 3497, 0));
    ...
    public long calculateFoePot() {
        ...
        for (GameItem item : prizePool) {
            if (item != null) {
                    int foeBurnRate = ShatteredShardPrice.getBurnPrice(null, item.getId(), false);
                if (foeBurnRate > 0) {
    ...
    public void plantSeed(Player person, boolean manualClick, boolean forced) {
        ...
        person.facePosition(new Position(person.getX(), person.getY() - 0));
        ...
        person.facePosition(person.absX, person.absY - 0);
        person.startAnimation(827);
        other.startAnimation(827);
