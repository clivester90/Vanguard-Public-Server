...
import io.runescape.content.Sounds;
import io.runescape.content.UimStorageChest;
...
import io.runescape.content.combat.weapon.WeaponDataConstants;
...
public class ItemAssistant {
	...
	public boolean isWearingAllItems(List<Integer> itemIDs) {
		Boolean[] state = new Boolean[itemIDs.size()];
		Arrays.fill(state,false);

		int index = 0;
		for (Integer item : itemIDs) {
			state[index] = isWearingItem(item);
			index++;
		}
		return Arrays.stream(state).allMatch(Boolean::valueOf);
	}
	...
	public void addItemToBankOrDrop(int itemId, int amount) {
		...
		player.sendMessage("<shad>@bl3@"+ getItemName(itemId) + " x" + item.getAmount() + " has been sent to your bank!");
	}
	...
	public boolean is2handed(String itemName, int itemId) {
		...
		if (itemName.contains("godsword") || itemName.contains("crystal") || itemName.contains("aradomin sword")
				...
				itemName.contains("abyssal bludgeon") || itemName.contains("spade") || itemName.contains("bow of") || itemName.contains("casket") ||
	...
	public void deleteArrow() {
		...
		if (player.getItems().isWearingItem(12926) || (player.getItems().isWearingItem(9174))) {
			return;
		}
	...
	public void handleAmethyst() {
		if (!player.boltTips&&!player.arrowTips&&!player.dartTips) {
			...
			if (player.playerLevel[Skill.FLETCHING.getId()] < 90) {
				player.sendMessage("You need 90 fletching to do this.");
				return;
			}
			player.getItems().deleteItem(21347, 1);
