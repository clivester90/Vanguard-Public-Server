...
import io.runescape.content.combat.effects.damageeffect.impl.DragonfireShieldEffect;
...
import io.runescape.content.skills.Fishing;
...
import io.runescape.content.skills.crafting.CraftingData;
import io.runescape.content.skills.crafting.Enchantment;
import io.runescape.content.skills.mining.Mineral;
...
import io.runescape.content.skills.smithing.Smelting.Bars;
...
import io.runescape.model.cycleevent.DelayEvent;
import io.runescape.model.cycleevent.impl.WheatPortalEvent;
...
import io.runescape.model.entity.player.broadcasts.BroadcastType;
...
import io.runescape.model.lobby.Lobby;
...
import io.runescape.net.outgoing.messages.ComponentVisibility;
...
//		if damageHeartTimer == 0 {
//			damageHeartTimer = 300;
//		} else {
//			c.sendMessage("already active"); //smth like this but idk how timers work in terms of decrementing etc
//
//		}
//	}
...
//	updateConfig(c, spell);
	...
	public void movePlayer(int x, int y, int h) {
		...
		c.teleTimer = 2;
		requestUpdates();
		Barrows.resetCoffinStatus(c);
	}

	public boolean movePlayerNew(int x, int y, int h) { // for the tournaments as it needed to be an boolean
		if (c.getPosition().inDuelArena() || Server.getMultiplayerSessionListener().inAnySession(c)) {
			if (!c.isDead) {
				return false;
			}
	...
	public void object(WorldObject object) {
		...
		if (c.getOutStream() != null) {
			...
			c.getOutStream().writeByteC((object.type << 2) + (object.getFace() & 3));
			c.getOutStream().writeByte(0);

			if (object.id != -1) { // removing
				c.getOutStream().createFrame(151);
				c.getOutStream().writeByteS(0);
				c.getOutStream().writeWordBigEndian(object.id);
				c.getOutStream().writeByteS((object.type << 2) + (object.getFace() & 3));
			}
			c.flushOutStream();
		}
	...
	public int achievementCompleted() {
		int completed = 0;
		for (AchievementList achievement : c.getPlayerAchievements().keySet()) {
			if (achievement != null && c.getPlayerAchievements().get(achievement) == achievement.getCompleteAmount()) {
				completed++;
			}
		}
		return completed;
...
//	 * @param offY The offset y of the projectile
	...
	public void createPlayersProjectile(NPC npc, Position to, int angle, int speed, int gfxMoving, int startHeight, int endHeight, int lockon, int time, int slope) {
		...
		NpcDef definition = NpcDef.getDefinitions().get(npc.getNpcId());
		if (definition != null && definition.getSize() > 2) {
			nX += Math.ceil(definition.getSize() / 2D);
			nY += Math.ceil(definition.getSize() / 2D);
		}
		int pX = to.getX();
		int pY = to.getY();
		int offX = (nY - pY) * -1;
		...
		switch (alch) {
			case "low":
				if (System.currentTimeMillis() - c.alchDelay > 1000) {
					...
					int amount2 = ShopAssistant.getItemShopValue(itemId) * 2;
	...
	public boolean canTeleport(String teleportType) {
		...
		if (c.getPosition().inClanWars() || c.getPosition().inClanWarsSafe()) {
			c.sendMessage("@cr10@You can not teleport from here, speak to the portal to leave.");
			return false;
		}
	...
	public void followNpc() {
		...
		if (c.combatFollowing) {
			...
			//|| c.getCombatItems().usingFaerdhinen();
			if (!projectile
