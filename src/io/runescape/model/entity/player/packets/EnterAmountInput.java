...
import io.runescape.content.skills.TabletCreation;
...
public class EnterAmountInput implements PacketType {
	...
	public void processPacket(Player c, int packetType, int packetSize) {
		...
		if (c.wogw) {//Well of good will
			c.wogwDonationAmount = Xamount;
			Wogw.donate(c, (int) c.wogwDonationAmount);
			c.wogw = false;
		}

		if (c.lottery) {//Lottery
			Lottery.receiveLotteryBuyAmount(c, c.getAmountInterface(), Xamount);
			c.lottery = false;
		}
		...
		if (c.moneyPouchNew) {//Money pouch
			c.getPouch().withdrawPouch(Xamount);
			c.moneyPouchNew = false;
		}

		if (c.moneyPouchTickets) {//Money pouch tickets
			c.getPouch().withdrawTickets(Xamount);
			c.moneyPouchTickets = false;
		}
		...
		case 45542:
			PriceChecker.withdrawItem(c, c.priceItem[c.xRemoveSlot], c.xRemoveSlot,
						Xamount);
			break;
