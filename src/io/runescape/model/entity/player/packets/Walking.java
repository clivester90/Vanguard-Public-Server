...
public class Walking implements PacketType {
	...
	public void processPacket(Player c, int packetType, int packetSize) {
		...
		if (c.getInferno() != null && c.getInferno().cutsceneWalkBlock)
			return;
		if (c.morphed) {
			c.sendMessage("You have transformed back into an human.");
		...
		if (Boundary.isIn(c, Boundary.SEA_SNAKE_OUTSIDE_DECK) || Boundary.isIn(c, Boundary.SEA_SNAKE_OUTSIDE_DECK2)) {//FIX FOR WHEN SNAKES MOVE PLAYERS IN WATER
			c.getPA().movePlayer(1684, 5920, 0);
			c.sendMessage("@red@You were moved back to the deck as the snake pulled you in the water.");
		}
