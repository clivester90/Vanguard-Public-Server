...
import io.runescape.content.dialogue.impl.IronmanNpcDialogue;
...
public class NpcOptionTwo {
	...
	public static void handleOption(Player player, int npcType) {
		...
		case 3362:
		case 2147://Daily tasks store
			player.getShops().openShop(205);
			break;

		case 2888://High alch store
			player.getShops().openShop(206);
			break;

		case 2184://TOKUL STORE
			player.getShops().openShop(201);
			break;

		case Npcs.DONATOR_SHOP:
			//player.sendMessage("Use @red@::claim to claim your donation.");
			if (System.currentTimeMillis() - player.lastDonation < 6000) {
				player.sendMessage("Please wait a few seconds between claiming donations.");
				return;
			}
			player.getDH().sendNpcChat1("Checking for unclaimed donations...", Npcs.DONATOR_SHOP, "Donator shop");
			//new Thread(new Donation(player)).run();
			player.lastDonation = System.currentTimeMillis();
			break;

		case Npcs.VOTE_SHOP:
			player.sendMessage("Use @red@::reward 1 all to claim your votes.");
			break;
		...
		case 6970:
			...
			case 3252:
				...
				break;
			case 3297:
				player.getThieving().steal(Thieving.Pickpocket.ARDOUGNE_KNIGHT, npc);
				break;
			case 8853:
