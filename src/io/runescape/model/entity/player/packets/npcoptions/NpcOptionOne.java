package io.runescape.model.entity.player.packets.npcoptions;

import java.util.*;

import io.runescape.Configuration;
import io.runescape.Server;
import io.runescape.content.PetCollector;
...
import io.runescape.content.bosses.nightmare.NightmareActionHandler;
import io.runescape.content.dailyrewards.DailyRewardsDialogue;
import io.runescape.content.dialogue.DialogueBuilder;
import io.runescape.content.dialogue.impl.*;
import io.runescape.content.minigames.inferno.Inferno;
import io.runescape.content.minigames.tob.TobConstants;
import io.runescape.content.miniquests.magearenaii.dialogue.KolodionDialogue;
import io.runescape.content.referral.EnterReferralDialogue;
import io.runescape.content.skills.*;
import io.runescape.content.skills.crafting.Tanning;
import io.runescape.content.skills.farming.FarmingTeleport;
...
import io.runescape.model.entity.npc.pets.Probita;
...
import io.runescape.model.entity.player.mode.group.GroupIronmanDialogue;
import io.runescape.model.entity.player.mode.group.contest.GroupIronmanContest;
...
public class NpcOptionOne {
	...
	public static void handleOption(Player player, int npcType) {
		...
		case 4420://tournament
			player.getDH().sendOption2("Enter the tournament","Maybe next time..");
			player.dialogueAction = 8906;
			break;

		case 1017://new lottery c.t
			player.getDH().sendDialogues(1593, npcType);
			break;

		case 3362:
		case 2147://Daily tasks
			player.getDH().sendDialogues(1900, npcType);
			break;
		...
		case 5567://death npc
			if(!player.getMode().isUltimateIronman()) {
				player.sendMessage("@red@Only ultimate iron-man are able to use this feature.");
				return;
			}
			player.getDH().sendDialogues(6767, 5567);
			break;




		case 1671://Strange old man at barrows
			if (player.getItems().playerHasItem(952)) {
				player.sendMessage("@red@You already have a spade.");
			    return;
			}
			player.getItems().addItem(952, 1);
			player.sendMessage("@red@The strange old man gives you a spade.");
            break;


		case 2184://TOKUL STORE
			player.getDH().sendDialogues(5700, 2184);
			break;
		...
		case 2850:
			...
			player.sendMessage("Find out more info about the contest at ::discord");
			break;
		case 8686:
		...
		case 9011:
			...
			player.sendMessage("@red@ Please type in ::vote to go to the site. And do ::reward 1 all to receive them!");
		...
		case 9120:
			...
			player.sendMessage("You currently have @red@"+player.amDonated+" donation points.");
			break;
		...
		case 4305:
			...
			break;
			case 2540:
				player.getThieving().steal(Thieving.Pickpocket.HAM_MEMBER, npc);
				break;
		case 5730:
			if (Boundary.isIn(player, Boundary.ARDOUGNE_MASTER)) {
