...
import io.runescape.achievements.*;
import io.runescape.content.*;
import io.runescape.content.SigilSystem1.tier1Sigils;
...
import io.runescape.content.collection_log.CollectionRewards;
import io.runescape.content.combat.magic.CombatSpellData;
import io.runescape.content.combat.magic.MagicRequirements;
import io.runescape.content.combat.magic.NonCombatSpellData;
...
import io.runescape.content.dialogue.DialogueActionButton;
import io.runescape.content.dialogue.DialogueConstants;
...
import io.runescape.content.items.Lamps;
...
import io.runescape.content.skills.Cooking;
import io.runescape.content.skills.Skill;
import io.runescape.content.skills.crafting.*;
import io.runescape.content.skills.crafting.CraftingData.tanningData;
import io.runescape.content.skills.slayer.SlayerRewardsInterface;
...
import io.runescape.model.entity.player.packets.dialogueoptions.*;
import io.runescape.model.items.ContainerUpdate;
import io.runescape.model.items.GameItem;
import io.runescape.model.items.bank.BankItem;
import io.runescape.model.items.bank.BankTab;
...
import io.runescape.model.multiplayersession.flowerpoker.FlowerPokerSession;
import io.runescape.model.multiplayersession.trade.TradeSession;
import io.runescape.model.shops.ShopAssistant;
import io.runescape.new_quest_tab.Tabswitcher;
...
public class ClickingButtons implements PacketType {
	...
	public void processPacket(final Player c, int packetType, int packetSize) {
		...
		if(c.getUpgradeHandler().button(actionButtonId)) {
			return;
		}
		...
		if (c.getTabswitcher().clickButton(actionButtonId)) {//For the new quest tab
			return;
		}

		if (tier1Sigils.handleButton(c, actionButtonId)) {//TIER 1 PERKS
			return;
		}
		...
		if (CrystalCaveEntryDialogue.handleTeleportButtons(c, actionButtonId)) {//Crystal cave teleports
			return;
		}
		...
		switch (actionButtonId) {
			...
			case 236094://sigil syster tiers locked
			case 236098://sigil syster tiers locked
			case 236102://sigil syster tiers locked
			case 236106://sigil syster tiers locked
				c.sendMessage("@red@Soon to be released to "+Configuration.SERVER_NAME+".");
				break;

			case 236090://Tier 1 sigils
			    SigilSystem.openInterface(c);
			    tier1Sigils.openInterface(c);
			    break;

			case 236086://remove window for sigil system
				c.getPA().removeAllWindows();
				break;

			case 19147:// closebutton on price checker
				if (c.xInterfaceId == 45500 || c.isChecking) {
					PriceChecker.clearConfig(c);
				}
				break;

			case 177193://Price checker deposit all
				PriceChecker.store(c);
				break;
			case 178010://Price checker withdraw all
				PriceChecker.clearConfig(c);
				break;
			case 177196://price checker search option
			...
			case 170186://back to quest tab from the staff tab
				c.sendMessage("@red@Staff tab has been turned off.");
				c.setSidebarInterface(2, QuestTab.INTERFACE_ID);
				break;

			case 170200://staff tab
				c.staffTab = true;
				break;
			case 170201://staff tab
				c.staffTab1 = true;
				break;
			case 170202://staff tab
				c.staffTab2 = true;
				break;
			case 170203://staff tab
				c.staffTab3 = true;
				break;
			case 170204://staff tab
				c.staffTab4 = true;
				break;
			case 170205://staff tab
				c.staffTab5 = true;
				break;
			case 170206://staff tab
				c.staffTab6 = true;
				break;
			case 170207://staff tab
				c.staffTab7 = true;
				break;
			case 170208://staff tab
				c.staffTab8 = true;
				break;
			case 170209://staff tab
				c.staffTab9 = true;
				break;
			case 170210://staff tab
				c.staffTab10 = true;
				break;
			case 170211://staff tab
				c.staffTab11 = true;
				break;
			case 170212://staff tab
				c.staffTab12 = true;
				break;
			case 170213://staff tab
				c.staffTab13 = true;
				break;
			case 170214://staff tab
				c.staffTab14 = true;
				break;
			case 170215://staff tab
				c.staffTab15 = true;
				break;
			case 170216://staff tab
				c.staffTab16 = true;
				break;
			case 170217://staff tab
				c.staffTab17 = true;
				break;
			case 170218://staff tab
				c.staffTab18 = true;
				break;
			case 170219://staff tab
				c.staffTab19 = true;
				break;
			...
			case 162038:
			case 162036:
				c.setProfilePrivacy(true);
				c.sendMessage("@dre@You have disabled your privacy settings.");
				break;

			case 162037:
			case 162035:
				c.setProfilePrivacy(false);
				c.sendMessage("@dre@You have enabled your privacy settings.");
				break;

			case 164006:
			case 164009:
				PlayerProfiler.manageReputation(c, c.viewing, actionButtonId);
				break;

			case 162039://my profile
				PlayerProfiler.myProfile(c);
				break;

			case 162032://search for player
				c.playerProfile = true;
				break;
			...
			case 74172://help quest tab
				...
				if (staff.size() > 0) {
					String message = "@blu@[Help] " + c.getDisplayName()
					...
					c.sendMessage("@red@Please try contacting a staff on the discord and they will respond ASAP.");
					c.sendMessage("@red@You can also type ::help to report something.");
				}
				break;


			case 27089://Tele orb last location
			...
			case 108012:
				if (c.petSummonId > 0) {
					//c.getDH().sendOption2("Call follower", "Pick-up pet");
					c.getDH().sendOption3("Call follower", "Pick-up pet", "Use unique-effect");
					c.dialogueAction = 85252;
				} else {
					c.sendMessage("You have no active pet!");
				}
...
//			break;
		...
		case 19236:
			...
			break;
		case 108003://price guide
			PriceChecker.open(c);
			c.sendMessage("@blu@You can also use the trading post for a price guide.");
			break;
		case 57057://bounty target teleport
		...
		case 117112://lunar spell moon clan land
			if (c.getItems().playerHasItem(557, 2) || c.getItems().playerHasItem(563, 1) || c.getItems().playerHasItem(9075, 2)) {
				c.getPA().startTeleport(2929, 4813, 0, "lunar", false);//Essense mine
				c.getItems().deleteItem(557, 2);// EARTH RUNE
				c.getItems().deleteItem(563, 1);// LAW RUNE
				c.getItems().deleteItem(9075, 2);// ASTRAL RUNE
				break;
			}
			c.sendMessage("You do not have the required runes to cast this spell.");
			break;

		case 117131://lunar spell ourania teleport
			if (c.getItems().playerHasItem(557, 6) || c.getItems().playerHasItem(563, 1) || c.getItems().playerHasItem(9075, 2)) {
				c.getPA().startTeleport(2453, 3231, 0, "lunar", false);// ZMI ALTAR
				c.getItems().deleteItem(557, 6);// EARTH RUNE
				c.getItems().deleteItem(563, 1);// LAW RUNE
				c.getItems().deleteItem(9075, 2);// ASTRAL RUNE
				break;
			}
		...
		case 117154://lunar spell water birth teleport
			if (c.getItems().playerHasItem(555, 1) || c.getItems().playerHasItem(563, 1) || c.getItems().playerHasItem(9075, 2)) {
				c.getPA().startTeleport(3603, 3529, 0, "lunar", false);//Port phasmatys
				c.getItems().deleteItem(555, 1);// WATER RUNE
				c.getItems().deleteItem(563, 1);// LAW RUNE
				c.getItems().deleteItem(9075, 2);// ASTRAL RUNE
				break;
			}
		...
		case 117186://lunar spell barbarian teleport
			if (c.getItems().playerHasItem(554, 3) || c.getItems().playerHasItem(563, 2) || c.getItems().playerHasItem(9075, 2)) {
				c.getPA().startTeleport(3056, 3310, 0, "lunar", false);//Falador
				c.getItems().deleteItem(554, 3);// FIRE RUNE
				c.getItems().deleteItem(563, 2);// LAW RUNE
				c.getItems().deleteItem(9075, 2);// ASTRAL RUNE
				break;
			}
			c.sendMessage("You do not have the required runes to cast this spell.");
		...
		case 117210://lunar spell kahzard teleport
			if (!(c.playerLevel[Skill.WOODCUTTING.getId()] >= 90)) {
				c.sendMessage("You would need an woodcutting level of 90 to teleport here.");
				return;
			}
			if (c.getItems().playerHasItem(555, 4) || c.getItems().playerHasItem(563, 2) || c.getItems().playerHasItem(9075, 2)) {
				c.getPA().startTeleport(1589, 3483, 0, "lunar", false);//Woodcutting guild
				c.getItems().deleteItem(555, 4);// WATER RUNE
				c.getItems().deleteItem(563, 2);// LAW RUNE
				c.getItems().deleteItem(9075, 2);// ASTRAL RUNE
				break;
			}
		...
		case 118018://lunar spell fishing guild teleport
			if (!(c.playerLevel[Skill.MINING.getId()] >= 92)) {
				c.sendMessage("You would need an mining level of 92 to teleport here.");
				return;
			}
			if (c.getItems().playerHasItem(555, 3) || c.getItems().playerHasItem(563, 3) || c.getItems().playerHasItem(9075, 3)) {
				c.getPA().startTeleport(3021, 9716, 0, "lunar", false);//Mining Guild
				c.getItems().deleteItem(555, 3);// WATER RUNE
				c.getItems().deleteItem(563, 3);// LAW RUNE
				c.getItems().deleteItem(9075, 3);// ASTRAL RUNE
				break;
			}
			c.sendMessage("You do not have the required runes to cast this spell.");
			break;
		...
		case 118042://lunar spell catherby teleport
			if (c.getItems().playerHasItem(555, 10) || c.getItems().playerHasItem(563, 3) || c.getItems().playerHasItem(9075, 3)) {
				c.getPA().startTeleport(2815, 3462, 0, "lunar", false);//Catherby
				c.getItems().deleteItem(555, 10);// WATER RUNE
				c.getItems().deleteItem(563, 3);// LAW RUNE
				c.getItems().deleteItem(9075, 3);// ASTRAL RUNE
		...
		case 118058://lunar spell ice platu teleport
			if (c.getItems().playerHasItem(555, 8) || c.getItems().playerHasItem(563, 3) || c.getItems().playerHasItem(9075, 3)) {
				c.getPA().startTeleport(2968, 3892, 0, "lunar", false);//ice warriors
				c.getItems().deleteItem(555, 8);// WATER RUNE
				c.getItems().deleteItem(563, 3);// LAW RUNE
				c.getItems().deleteItem(9075, 3);// ASTRAL RUNE
				break;
			}
...
//			}
		...
		case 33206:
			if (c.getPosition().inClanWarsSafe()) {
				...
				if (c.wildLevel > 0) {
					c.sendMessage("@red@You cant use this in the wilderness.");
					return;
				}
				...
				c.getDH().sendOption3("Monster zone", "Rock crabs", "-- Cancel --");
		...
		case 33218:
			if (c.getPosition().inClanWarsSafe()) {
				...
				//c.getSI().selected = 4;
				c.getDH().sendOption3("Edgeville altar", "Wilderness altar", "-- Cancel --");
		...
		case 33224: // runecrafting
			...
			c.getDH().sendOption3("Zmi", "Abyss", "-- Cancel --");
		...
		case 33210: // agility
			...
			c.getDH().sendOption2("Talk to grace for teleports","-- Cancel --");
		...
		case 33213: // herblore
			...
			c.getDH().sendOption2("Zahur","-- Cancel --");
		...
		case 33216: // theiving
			...
			c.getDH().sendOption2("Edgeville stalls","-- Cancel --");
		...
		case 33219: // crafting
			...
			c.getDH().sendOption2("Crafting and tanner","-- Cancel --");
		...
		case 33222: // fletching
			...
			c.getDH().sendOption2("Skilling island bank","-- Cancel --");
		...
		case 47130:// slayer
			...
			c.getDH().sendOption3("Slayer masters", "Konar slayer","-- Cancel --");
		...
		case 33208:// mining
			...
			c.getDH().sendOption2("Skilling island","-- Cancel --");
		...
		case 54104: // farming
			...
			c.getDH().sendOption2("Falador","-- Cancel --");
