...
import io.runescape.content.combat.weapon.WeaponData;
import io.runescape.content.commands.CommandManager;
import io.runescape.content.event.eventcalendar.EventCalendarHelper;
import io.runescape.content.items.Starter;
import io.runescape.content.minigames.bounty_hunter.TargetState;
import io.runescape.content.minigames.inferno.Inferno;
import io.runescape.content.skills.Skill;
import io.runescape.content.skills.farming.Plants;
...
import io.runescape.sql.eventcalendar.queries.AddWinnerQuery;
import io.runescape.util.Misc;
import io.runescape.util.logging.player.ClanChatLog;
import io.runescape.util.logging.player.CommandLog;
...
//        }
            ...
            if (playerCommand.startsWith("foe") && c.getRights().isOrInherits(Right.OWNER)) {
                if (Boundary.isIn(c, Boundary.EDGEVILLE_PERIMETER)) {
                    c.sendMessage("@bla@[@SS@FoE@bla@]@blu@ Remember, any exchanges are @red@final@blu@, items will not be returned.");
                    c.sendMessage("@bla@[@SS@FoE@bla@] @blu@Click an item in your inventory to offer. Use the green arrow to confirm.");
                    ...
                    if (c.getItems().playerHasItem(771)) {
                        c.sendMessage("@bla@[@red@SS@bla@]@blu@The @cya@ancient branch@blu@ must be used with only 1 item at a time.");
                    }
                } else {
            ...
            if (playerCommand.startsWith("reward")) {
                ...
                com.everythingrs.vote.Vote.service.execute(() -> {
                    try {
                        ...
                        PlayerHandler.executeGlobalMessage("<img=20> <col=dbffba><shad=1>" + c.getLoginName() + " has just voted for Vanguard!");
                        PlayerHandler.executeGlobalMessage("<img=20> <col=dbffba><shad=1> Support the server by typing ::vote daily!");
