...
import io.runescape.content.achievement_diary.impl.*;
import io.runescape.content.barrows.Barrows;
import io.runescape.content.bosses.*;
import io.runescape.content.bosses.godwars.God;
import io.runescape.content.bosses.hydra.AlchemicalHydra;
import io.runescape.content.bosses.kratos.KratosNpc;
import io.runescape.content.combat.Hitmark;
import io.runescape.content.dialogue.impl.CrystalCaveEntryDialogue;
import io.runescape.content.dialogue.impl.FireOfDestructionDialogue;
import io.runescape.content.dialogue.impl.SkillingPortalDialogue;
import io.runescape.content.hs.NexHighscore;
import io.runescape.content.hs.XericHighscore;
...
import io.runescape.content.minigames.raids.CoxParty;
import io.runescape.content.minigames.raids.Raids;
import io.runescape.content.skills.FlaxPicking;
import io.runescape.content.skills.Skill;
import io.runescape.content.skills.agility.AgilityHandler;
import io.runescape.content.skills.agility.impl.rooftop.RooftopArdougne;
import io.runescape.content.skills.agility.impl.rooftop.RooftopSeers;
import io.runescape.content.skills.agility.impl.rooftop.RooftopVarrock;
import io.runescape.content.skills.crafting.BraceletMaking;
import io.runescape.content.skills.crafting.JewelryMaking;
import io.runescape.content.skills.hunter.Hunter;
import io.runescape.content.skills.runecrafting.Runecrafting;
import io.runescape.content.skills.smithing.CannonballSmelting;
import io.runescape.content.skills.thieving.Thieving.Stall;
import io.runescape.content.skills.woodcutting.Tree;
...
import io.runescape.model.collisionmap.doors.DoorHandler;
...
import io.runescape.model.entity.player.packets.objectoptions.impl.*;
...
import io.runescape.model.multiplayersession.duel.DuelSessionRules.Rule;
import io.runescape.model.world.objects.GlobalObject;
import io.runescape.util.Location3D;
...
public class ObjectOptionOne {
	...
	public static void handleOption(final Player c, int objectType, int obX, int obY) {
	    ...
	    if (Barrows.isBarrowsObject(c, objectType)) {
			return;
		}
	...
	//				} else {
		//				c.sendMessage("Please re-log and report this issue to staff...");
	//				}
	//			} else {
	...
	//			break;
			//Lumbridge Objects
			case 5492: //HAM hideout
				c.sendMessage("You open the trap door, and climb down.");
				c.getPA().movePlayer(3160, 9633, 0);
				break;
//End lumbridge objects
			case 23922:
			case 41807://exit from sea snake
				c.sendMessage("@red@You have teleported from the powerful sea snake.");
				c.getPA().movePlayer(3102, 3510, 0);
				break;

			case 30944: //creation of the ancient wyvernn shield
				if (c.getPA().getLevelForXP(c.playerXP[Skill.SMITHING.getId()]) < 66) {
					c.sendMessage("You need a smithing level of 66 to do this.");
					return;
				}
				if (c.getPA().getLevelForXP(c.playerXP[Skill.MAGIC.getId()]) < 66) {
					c.sendMessage("You need a magic level of 66 to do this.");
					return;
				}
				if (c.getItems().playerHasItem(21637) && c.getItems().playerHasItem(2347) && c.getItems().playerHasItem(2890)) {
					c.startAnimation(898);
					c.getItems().deleteItem(2890, 1); //elemental shield
					c.getItems().deleteItem(21637, 1); //wyvern visage
					c.getItems().addItem(21633, 1); // ancient wyvern shield
					c.getDH().sendItemChat("","You manage to create an ancient wyvern shield.", 21633, 200, 14, 0);
					return;
				}
				c.getDH().sendItemChat("","You will need hammer, elemental shield, wyvern visage", 21637, 200, 14, 0);
				break;

			case 16664://wilderness dungeon entrance
				AgilityHandler.delayEmote(c, "CLIMB_DOWN", 3044, 10323, 0, 3);
				break;

			case 43868://rev caves new exit
				AgilityHandler.delayEmote(c, "CLIMB_UP", 3124, 3805, 0, 3);
				break;

			case 31558://rev caves new exit
				AgilityHandler.delayEmote(c, "CLIMB_UP", 3087,3655, 0, 3);
				break;

			case 40390://wilderness slayer cave enter
				AgilityHandler.delayEmote(c, "CRAWL", 3406, 10145, 0, 3);
				break;

			case 40391://wilderness slayer cave exit
				AgilityHandler.delayEmote(c, "CRAWL", 3293, 3749, 0, 3);
				break;

			case 40388://wilderness slayer cave enter
				AgilityHandler.delayEmote(c, "CRAWL", 3385, 10052, 0, 3);
				break;
			...
			case 42934://Nex entrance to lobby
					if (c.absX == 2900 || c.absX == 2901) {
						c.getDH().sendOption2("WARNING!", "Exit", "Cancel..");
						c.dialogueAction = 19130;
						return;
					}
				    c.getGodwars().enterBossRoom(God.ANCIENT);
				    break;

			case 42965://Nex altar
			...
			case 42967://Nex entrance to chamber
				if (Nex.activated) {
					Nex.respawnCheck();
					c.getDH().sendStatement("There are currently " +Boundary.getPlayersInBoundary(Boundary.NEX_BOSS_ROOM)+ " players fighting nex,", "Please wait for the next",
							"game to begin.");
					return;
				}
				if (c.absX == 2908) {
					...
					Nex.respawnCheck();
					c.sendMessage("@red@You have entered nex prison.");
					return;
				} else
					c.getDH().sendStatement("You can not leave this way, use the altar.");
				break;

			case 42933://Nex entrance to other godwars rooms
				c.getDH().sendStatement("You can not use this door.");
				break;

			case 42966://Ancient forge
				c.getDH().sendStatement("The forge is currently under construction check back soon.");
				break;


			case 13882:// kratos exit
			case 13878:// kratos exit
				KratosNpc.spawned = false;
				c.getPA().movePlayer(3101,3495,0);
				NPCHandler.despawn(6009, 0);//kratos
				break;

			case 12451://Kratos chest
				KratosChest.searchChest(c);
				break;


			case 27517://Afk range
				c.getDH().sendStatement("You can use all @red@noted raw fish@bla@ on the range for @red@xp/afk points.");
				break;

			case 10082://Afk furnace
				c.getDH().sendStatement("You can use all @red@noted bars@bla@ on the furnace for @red@xp/afk points.");
				break;



			case 4121://tarn razorlor reward chest
				Tarn.execute(c);
				break;

			case 15770://tarn no entrance
				c.sendMessage("@red@You cannot use this passage.");
				break;


			case 16130://tarn entrance
				c.getDH().sendOption2("Teleport home", "Cancel");
				c.dialogueAction = 16130;
				break;

			case 20539://tarn entrance
				if (c.getItems().playerHasItem(11040)) {
					AgilityHandler.delayEmote(c, "CRAWL", 3186, 4612, 0, 3);
					c.getItems().deleteItem(11040, 1); //tarn entry key
					return;
				} else
					c.getDH().sendStatement("@red@You need a Tarn Razorlor entry key to enter.");
				break;
			...
			case 26273://Loyalty chest
				c.getDH().sendOption2("Claim loyalty rewards", "Cancel");
				c.dialogueAction = 26273;
				break;


			case 36201://Zalcano entrance
				break;



			case 7127://xeric leaderboards
				c.setHighscore(new XericHighscore());
				c.getHighscore().process();
				c.getHighscore().generateList(c);
				break;


			case 29777://Chambers of xeric lobby entrance and exit
				...
				c.sendMessage("You will need to create/join an party to enter the chambers of xeric.");
			...
			//	if  (Boundary.isIn(c, Boundary.RAIDS_LOBBY)) {
				...
				//	c.getPA().movePlayer(1241, 3558, 0);
				//	return;
			//	}
			//	LobbyManager.get(LobbyType.CHAMBERS_OF_XERIC)
			...
			case 7321: //portal to repawn general graador
				if(KreeArraInstance.spawned) {
					c.sendMessage("Kill the first lot first you punk.");
					return;
				}
				KreeArraInstance.respawnAgain(c);
				break;


			case 7319: //portal to repawn general graador
				if(GeneralGraadorInstance.spawned) {
					c.sendMessage("Kill the first lot first you punk.");
					return;
				}
				GeneralGraadorInstance.respawnAgain(c);
				break;

			case 7318: //portal to repawn commander zilayana
				if(CommanderZilyanaInstance.spawned) {
					c.sendMessage("Kill the first lot first you punk.");
					return;
				}
				CommanderZilyanaInstance.respawnAgain(c);
				break;

			case 7315: //portal to repawn kril in instance area
				if(KrilTsutsarothInstance.spawned) {
					c.sendMessage("Kill the first lot first you punk.");
					return;
				}
				KrilTsutsarothInstance.respawnAgain(c);
				break;


			case 15343://koranian chest
				KoranianChest.searchChest(c);
				break;


			case 29067://LMS
				if (c.absX == 3116) {
					LobbyManager.get(LobbyType.L_M_S)
							.ifPresent(lobby -> lobby.attemptJoin(c));
					return;
				} else
					c.getDH().sendStatement("@red@You cannot leave wait for the timer to end.");
				break;

			case 13389://Seren chest
				if (c.getItems().playerHasItem(6792)) {
					new SerenChest().roll(c);
					return;
				} else
					c.getDH().sendStatement("@red@You need a seren key to open this chest.");
				break;
			...
			case 27269://Solak chest
				if (c.getItems().playerHasItem(22428)) {
					EventChestSolak.searchChest(c);
					return;
				} else
					c.getDH().sendStatement("@red@You need a solak key to open this chest.");
				break;

			case 3192://New highscores
				c.getDH().sendOption3("Tournament hiscores", "Live Pvp hiscores", "Live Pvm hiscores");
				c.dialogueAction = 3192;
				break;

			case 17436://upgrade table
				//UpgradeWellInterface.displayupGradeInterface(c);
				c.getUpgradeHandler().openInterface();
				break;

			case 14986:
				GlodChest.searchChest(c);
				break;
			...
			case 15477:
				...
				break;
			case 33320:
				c.sendMessage("@bla@[@red@SS@bla@]@blu@ Remember, any exchanges are @red@final@blu@, items will not be returned.");
				c.sendMessage("@bla@[@red@SS@bla@] @blu@Click an item in your inventory to offer. Use the green arrow to confirm.");
				...
				break;
			case 29778:
				c.getPA().movePlayer(1245, 3562, 0);
				c.setRaidsInstance(null);
				break;


			case 31623:
			case 32148: //CREATION FOR FEROCIOUS GLOVES
				if (c.getItems().playerHasItem(995, 5_000_000) && c.getItems().playerHasItem(22983) && c.getItems().playerHasItem(2347)) {
					c.startAnimation(898);
					c.getItems().deleteItem(22983, 1); //leather
					c.getItems().deleteItem(995, 5_000_000); //coins
					c.getItems().addItem(22981, 1); //ads forocious gloves
					c.getDH().sendItemChat("","You manage to form a pair of ferocious gloves.", 22981, 200, 14, 0);
					AchievementHandler.activate(c, AchievementList.CRAFT_FEROCIOUS_GLOVES, 1);//NEW ACHIEVEMNTS
					return;
				}
				c.getDH().sendItemChat("","You need a hammer, Hydra Leather & 5 million coins.", 22983, 200, 14, 0);
				break;
			...
			case 34554:
				...
				if (!c.getSlayer().getTask().get().getPrimaryName().equals("hydra")
					...
					if (c.absX == 1355) {
						...
						c.sendMessage("@red@You have chosen to leave the hydra instance.");
						c.getPA().movePlayer(1355, 10259, 0);
					}
				}
				break;
			/*
			...
			case 29150:
				...
				AchievementHandler.activate(c, AchievementList.CHANGING_PRAYERS, 1);//NEW ACHIEVEMENTS
			...
			case 34530://Karulm first and second floor
				if (c.absX == 1329 && c.absY == 10206) {
					c.getPA().movePlayer(1334, 10205, 1);
				} else if(c.absX == 1313 && c.absY == 10189) {
			...
			case 12768:
				...
				if (c.getItems().freeSlots() < 3) {
					c.getDH().sendStatement("@red@You need at least 3 free slots to open this.");
					return;
				}
				break;
			case 11845:
...
//					} else {
			...
			case 21598:
			case 21599:
				c.getPA().movePlayer(1935, 2718, 0);
				break;
			...
			case 17385:
				if (Boundary.isIn(c, Boundary.EDGE_DUNG_LADDER)) {
					...
					AgilityHandler.delayEmote(c, "CLIMB_UP", 3095, 3486, 0, 2);
				}
				break;
			case 33318:
			...
			case 30266:
				if (c.usedFc == true) {
					c.getPA().movePlayer(2495, 5111, 0);
				} else if (c.getItems().playerHasItem(6570, 1)) {
					c.getItems().deleteItem(6570, 1);
					c.usedFc = true;
					c.getPA().movePlayer(2495, 5111, 0);
				} else {
					c.sendMessage("@blu@You must @red@sacrifice @blu@your @red@firecape@blu@ at least once.");
					return;
				}
				break;
			...
			case 677:
				if (c.getX() <= 2970) {
					c.getDH().sendOption3("Normal room", "Ironman room", "-- Cancel --");
					c.dialogueAction = 67677;
				} else {
					c.getPA().movePlayer(2970, c.getY(), 2);
				}
				break;
			...
			case 13287:
				...
				if (!c.getMode().isBankingPermitted() && (c.getItems().playerHasItem(8866) || c.unlockedUltimateChest)) {
					...
					c.sendMessage("You must use a key from the SS shop to unlock this chest.");
				} else {
