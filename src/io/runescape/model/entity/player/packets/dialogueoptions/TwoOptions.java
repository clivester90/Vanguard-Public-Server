package io.runescape.model.entity.player.packets.dialogueoptions;
...
import io.runescape.content.HourlyRewardBox;
import io.runescape.content.bosses.VoteBoss;
import io.runescape.content.combat.Hitmark;
import io.runescape.content.dailytasks.TaskTypes;
import io.runescape.content.shatteredshards.ShatteredShards;
import io.runescape.content.LootValue;
import io.runescape.content.SkillcapePerks;
...
//import io.runescape.content.commands.all.Claim;
import io.runescape.content.items.Degrade;
import io.runescape.content.lootbag.LootingBag;
import io.runescape.content.minigames.barrows.RoomLocation;
...
import io.runescape.model.entity.player.PrayerScrolls;
...
public class TwoOptions {
	...
	public static void handleOption1(Player c) {  //see This handles option 1 if u scroll down ull see the section two
		...
		switch (c.dialogueAction) {
			...
			case 6799://Anicent coin
				int amount1 = c.getItems().getItemAmount(11180);
				if (c.getItems().playerHasItem(11180, amount1)) {
					c.getItems().deleteItem2(11180, amount1);
					c.getItems().addItem(18825, amount1);
					c.sendMessage("@red@You convert the coin and it manages to decrease in foe value by @red@30%.");
					c.getPA().closeAllWindows();
					return;
				}
				break;

			case 6701://Anicent talisman
				int amount = c.getItems().getItemAmount(681);
				if (c.getItems().playerHasItem(681, amount)) {
					c.getItems().deleteItem2(681, amount);
					c.getItems().addItem(16623, amount);
					c.sendMessage("@red@You convert the talisman and it manages to decrease in foe value by @red@30%.");
					c.getPA().closeAllWindows();
					return;
				}
				break;
			...
			case 19130://Nex exit
				...
				c.sendMessage("You have left the nex lobby,.");
				c.getPA().closeAllWindows();
				break;
			...
			case 1903://Daily tasks
				if (c.dailyEnabled) {
					c.playerChoice = TaskTypes.PVM;
					//c.dailyChoice = 1;
					c.getDH().sendDialogues(1906, 1909);
				} else {
			...
			case 16130://tarn exit
				c.getPA().movePlayer(3099, 3508, 0);
				c.getPA().closeAllWindows();
				break;
			...
			case 26273://Hourly chest
				...
				if (!c.loyaltyChestClaimable) {
					long milliseconds = (long) c.playTime1 * 600;
			...
			case 164:
				c.getPA().movePlayer(3551, 9689, 0);
				c.getPA().closeAllWindows();
				break;



			case 9810://falador teleport for farming skill
				c.getPA().startTeleport(3053, 3301, 0, "modern", false);
				c.getPA().closeAllWindows();
				break;

			case 9809://skilling island teleport for mining skill
				c.getPA().startTeleport(3803, 3539, 0, "modern", false);
				c.getPA().closeAllWindows();
				break;

			case 9807://skilling teleport for fletch skill
				c.getPA().startTeleport(3808, 3552, 0, "modern", false);
				c.getPA().closeAllWindows();
				break;

			case 9806://tanning and crafter teleport for craft skill
				c.getPA().startTeleport(3810, 3550, 0, "modern", false);
				c.getPA().closeAllWindows();
				break;

			case 9805://edge stalls teleport for theive skill
				c.getPA().startTeleport(3075, 3476, 0, "modern", false);
				c.getPA().closeAllWindows();
				break;

			case 9804://zahur teleport for herblore skill
				c.getPA().startTeleport(3425, 2910, 0, "modern", false);
				c.getPA().closeAllWindows();
				break;

			case 9803:// grace teleport for agilty skill
				c.getDH().sendDialogues(14400, 5919);
				break;

			case 5700://vote boss chest
				c.getPA().movePlayer(2437, 5125, 0);
				c.getPA().closeAllWindows();
				break;

			case 5679://vote boss chest
				if (c.combatLevel < 60) {
					c.sendMessage("You need a combat level of at least 60 to redeem the chest.");
					c.getPA().closeAllWindows();
					return;
				}
				c.sendMessage("@red@You have chosen to redeem the vote chest, and it suddenly vanishes.");
				c.sendMessage("@red@You have entered the vote boss instance.");
				c.getItems().deleteItem(8152, 1);
				VoteBoss.startVoteBoss(c);
				c.dialogueAction = -1;
				c.getPA().closeAllWindows();
				break;

			case 24675://upgrade table
				UpgradeTable.processUpgrade(c.upgradingItem, c);
				c.dialogueAction = -1;
				c.getPA().closeAllWindows();
				break;
		...
		case 130135:
			...
			ShatteredShards.exchangeItemForPoints(c);
			c.getPA().closeAllWindows();
	...
	public static void handleOption2(Player c) {
		...
		switch (c.dialogueAction) {
			...
			case 6799://Anicent coin cancel option
				c.getPA().closeAllWindows();
				break;


			case 8906://Tournament cancel option
				c.getPA().closeAllWindows();
				break;

			case 19130://Nex exit - cancel option
				c.getPA().closeAllWindows();
				break;
			...
			case 1903://Daily tasks
				if (c.dailyEnabled) {
					...
					//c.dailyChoice = 2;
					c.getDH().sendDialogues(1907, 1909);
				} else {
					c.sendMessage("You must enable Daily Tasks first!");
					c.getPA().closeAllWindows();
				}
			...
			case 16130://tarn cancel option
				c.getPA().closeAllWindows();
				break;


			case 26273://Hourly chest close option
				c.getPA().closeAllWindows();
				break;


			// Barrows, You have found a hidden tunnel, no cancel option!
		case 164:
			c.getPA().closeAllWindows();
			break;

		case 9810://cancel option for farming skill
			c.getPA().closeAllWindows();
			break;

		case 9809://cancel option for mining skill
			c.getPA().closeAllWindows();
			break;

		case 9807://cancel option for fletch skill
			c.getPA().closeAllWindows();
			break;

		case 9806://cancel option for craft skill
			c.getPA().closeAllWindows();
			break;

		case 9805://cancel option for theif skill
			c.getPA().closeAllWindows();
			break;

		case 9804:// cancel option for herblore skill
			c.getPA().closeAllWindows();
			break;

		case 9803:// cancel option for agilty skill
			c.getPA().closeAllWindows();
			break;
