...
import io.runescape.content.PriceChecker;
...
import io.runescape.model.ContainerAction;
import io.runescape.model.ContainerActionType;
...
public class ContainerAction1 implements PacketType {
	...
	public void processPacket(Player c, int packetType, int packetSize) {
		...
		switch (interfaceId) {
			...
			case 45542://Remove item from the price checker
		...
		case 33405:
			...
			if (ShatteredShardPrice.getBurnPrice(c, removeId, true) != -1  && c.getItems().playerHasItem(removeId)) {//checks if item is on foe list, and they actually have item.
				...
				int price = ShatteredShardPrice.getBurnPrice(c, removeId, true);
				int ironManPrice = (int) (price * 1.1);
				if (price == -1) {
					c.getPA().sendFrame126("@red@0", 33409);
					c.currentExchangeItem = -1;
				} else {
					if (c.getMode().isIronmanType() && ShatteredShards.canBurnWithBranch(c)) {
						price = ironManPrice;
					}
