package io.runescape.model.entity.player.mode.group;

import io.runescape.content.dialogue.*;
...
import io.runescape.model.items.ItemInterface;
import io.runescape.model.items.inventory.Inventory;
...
public class GroupIronmanDialogue extends DialogueBuilder {
    ...
    private Optional<GameItem> forfeitItemsIntoInventory(Player player, Inventory into, boolean dryRun) {
        ...
        Predicate<ItemInterface> forfeit = it -> it.getDef().isTradable() && (
                ShatteredShardPrice.getBurnPrice(null, it.getId(), false) != -1
    ...
    private boolean forfeitOnLeave(int itemId, int amount) {
        return ShatteredShardPrice.getBurnPrice(null, itemId, false) != 0 || itemId == Items.COINS && amount > 100_000;
    }
}
