...
import io.runescape.content.skills.slayer.*;
...
public class DialogueHandler {
    ...
    public void sendDialogues(int dialogue, int npcId) {
        ...
        switch (dialogue) {
            ...
            case 6799://Ancient coins
                sendOption2("Yes I couldn't care less!", "I would rather not.");
                c.dialogueAction = 6799;
                break;
            ...
            case 6701://talisman
                sendOption2("Yes I couldn't care less!", "I would rather not.");
                c.dialogueAction = 6701;
                break;


            case 1593:
                sendNpcChatAnimation("Greetings!", "Are you looking to participate in " + Configuration.SERVER_NAME + "'s lottery?", FacialAnimation.DEFAULT.getAnimationId());
                c.nextChat = dialogue + 1;
                break;

            case 1594:
                sendPlayerChat1("Lottery manager.. Haven't I seen you somewhere before?");
                c.nextChat = dialogue + 1;
                break;

            case 1595:
                sendNpcChatAnimation("Wha.. ahem... you must be thinking of someone else.", "Do you need anything?", FacialAnimation.HAPPY.getAnimationId());
                c.nextChat = dialogue + 1;
                break;

            case 1596:
                sendOption4("How does the lottery work?", "Buy lottery tickets for " + Misc.formatNumber(Lottery.getLotteryTicketCost()) + " each", "What is the current pot at?",
                        "What is my percentage of winning?");
                c.dialogueAction = 1596;
                break;
            case 1597:
                sendNpcChatAnimation("The winner takes all the pot!", "I do however take a small " + Lottery.COMMISION_PERCENTAGE + "% commission to cover the",
                ...
                break;
            case 1598:
                sendNpcChatAnimation("Buy as many tickets as you can so you have a greater %", "chance of winning!", FacialAnimation.HAPPY.getAnimationId());
                c.nextChat = 1597;
                break;


            case 1900:
                sendNpcChat4("Hello! Would you like to enable Daily Tasks?", "These tasks can be assigned and completed", "once a day for some pretty neat rewards!", "Enable Daily Tasks?", 3362, "Daily Task Manager");
                c.nextChat = 1901;
                break;
            case 1901:
                sendOption3("Yes, enable daily tasks", "No, disable daily tasks.", "I'd like to reset my daily task.");
                c.dialogueAction = 1901;
                break;
            case 1902:
                sendNpcChat2("Please select your preference on the type", "of tasks you would like to receive!", 3362, "Daily Task Manager");
                c.nextChat = 1903;
                break;
            case 1903:
                sendOption2("Choose PvM Related Task", "Choose Skilling Related Task");
                c.dialogueAction = 1903;
                break;

            case 1904:
                sendNpcChat2("You have enabled daily tasks! Please", "press continue to select a task preference.", 3362, "Daily Task Manager");
                c.nextChat = 902;
                break;
            case 1905:
                sendNpcChat2("You have disabled daily tasks! Talk to", "me if you change your mind.", 3362, "Daily Task Manager");
                c.nextChat = 0;
                break;

            case 1906:
                sendNpcChat2("Great, your tasks will now be PvM related", "until you change your mind!", 3362, "Daily Task Manager");
                c.nextChat = 0;
                DailyTasks.complete(c);
                DailyTasks.assignTask(c);
                break;
            case 1907:
                sendNpcChat2("Great, your tasks will now be Skilling related", "until you change your mind!", 3362, "Daily Task Manager");
                c.nextChat = 0;
                DailyTasks.complete(c);
                DailyTasks.assignTask(c);
                break;
            ...
            case 6768://death npc
                sendOption2("Yes I'm fearless!", "No please dont kill me.");
                c.dialogueAction = 6768;
                break;
            ...
            case 164:
                ...
                c.dialogueAction = 164;
                break;

            case 5750:// Godwars Instance
                sendOption4("Zamorak (@red@6m)", "Saradomin(@red@6m)", "Bandos(@red@7m)", "Armadyl(@red@7m)");
                c.dialogueAction = 5751;
                break;

            case 5700://Inferno
                sendOption2("Try my luck at the inferno.", "Actually.. forget that i dont think im good enough.");
                c.dialogueAction = 5700;
                break;


            case 5678://vote chest
                c.talkingNpc = 9011;
                sendNpcChat("When redeeming the vote boss chest", "you will be teleported to an secret", "location and the chest will vanish.");
                c.nextChat = 5679;
                break;

            case 5679://vote chest
                sendOption2("Yes! Let me have a try at the vote boss.", "Actually.. forget that ill try another time.");
                c.dialogueAction = 5679;
                break;


            case 24675://upgradetable
                sendOption2("Yes! Let me have a try", "Actually.. forget that I like my items.");
                c.dialogueAction = 24675;
                break;
            ...
            case 675:
                ...
                sendNpcChat("This is the @red@Fire of Exchange@bla@.", "Use items on it to earn shards",
    ...
    private void sendStartInfo() {
        c.getPA().sendFrame126("Continue?", 6180);
        c.getPA().sendFrame126("As you collect your reward, you notice an awful smell.", 6181);
        c.getPA().sendFrame126("You look below the remaining debris to the bottom of the", 6182);
        c.getPA().sendFrame126("chest. You see a trapdoor. You open it and it leads to a ladder", 6183);
        c.getPA().sendFrame126("that goes down a long ways.", 6184);
        c.getPA().sendChatboxInterface(6179);
    }

    private void sendStartInfoPin() {
        c.getPA().sendFrame126("", 6180);
        c.getPA().sendFrame126("Since you don't know your P I N, it will be deleted in @red@3 days@bla@. If you", 6181);
        c.getPA().sendFrame126("wish to cancel this change, you may do so by entering your P I N", 6182);
        c.getPA().sendFrame126("correctly next time you attempt to use your bank.", 6183);
    ...
    public void sendItemChat(String header, String one, int item, int zoom, int offset1, int offset2) {
        c.sendMessage(":packet:senditemchat 307 " + offset1 + " " + offset2);
    ...
    public void sendNpcChatAnimation(String s1, String s2, int expression) {
        c.getPA().sendFrame200(4888, expression);
        c.getPA().sendFrame126(NpcDef.forId(c.talkingNpc).getName(), 4889);
    ...
    public void sendNpcChatAnimation(String s, int expression) {
        c.getPA().sendFrame200(4883, expression);
        c.getPA().sendFrame126(NpcDef.forId(c.talkingNpc).getName(), 4884);
        c.getPA().sendFrame126(s, 4885);
        c.getPA().sendFrame75(c.npcType, 4883);
        c.getPA().sendFrame164(4882);
    }

    public void sendNpcChatAnimation(String s1, String s2, String s3, String s4, int expression) {
        c.getPA().sendFrame200(4901, expression);
        c.getPA().sendFrame126(NpcDef.forId(c.talkingNpc).getName(), 4902);
        c.getPA().sendFrame126(s1, 4903);
        c.getPA().sendFrame126(s2, 4904);
        c.getPA().sendFrame126(s3, 4905);
        c.getPA().sendFrame126(s4, 4906);
