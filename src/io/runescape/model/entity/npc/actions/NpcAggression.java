...
public class NpcAggression {
    ...
    private static int getAggressionDistance(NPC npc, Player player, int baseDistance) {
        ...
        if (Boundary.isIn(npc, Boundary.NEX_BOSS_ROOM) && npc.getNpcId() > 0) {
            return 45;
        }
        if (Boundary.isIn(npc, Boundary.SARACHNIS_LAIR) || Boundary.isIn(npc, Boundary.MIMIC_LAIR) || Boundary.isIn(npc, Boundary.KRATOS_AREA) || Boundary.isIn(npc, Boundary.GROTESQUE_LAIR)) {
            return 25;
        }
        if (npc.getNpcId() > 0)
            switch (npc.getNpcId()) {
    ...
    public static boolean isAggressive(NPC npc) {
        ...
        if (Boundary.isIn(npc, Boundary.GODWARS_BOSSROOMS)
                ...
                || Boundary.isIn(npc, Boundary.KRATOS_AREA) && npc.getBehaviour().isAggressive()
