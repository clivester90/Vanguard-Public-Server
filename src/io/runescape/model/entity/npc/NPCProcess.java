...
import io.runescape.content.Tarn;
...
import io.runescape.content.combat.death.NPCDeath;
import io.runescape.content.koranian_event.KoranianEventBossHandler;
import io.runescape.content.minigames.inferno.AncestralGlyph;
...
import io.runescape.model.entity.npc.actions.NPCHitPlayer;
import io.runescape.model.entity.npc.actions.NpcAggression;
import io.runescape.model.entity.npc.data.RespawnTime;
...
public class NPCProcess {
    ...
    private void processing() {
        ...
        if (npc.getNpcId() == 6610) {//vennatis
            npc.getBehaviour().isAggressive();
            npc.getBehaviour().setRespawn(true);
        }
        if (npc.getNpcId() == 6615) {//scorpia
            npc.getBehaviour().isAggressive();
            npc.getBehaviour().setRespawn(true);
        }
        if (npc.getNpcId() == 7936) {//rev
            npc.getBehaviour().isAggressive();
            npc.getBehaviour().setRespawn(true);
        }
        if (npc.getNpcId() == 7940) {//rev
            npc.getBehaviour().isAggressive();
            npc.getBehaviour().setRespawn(true);
        }
        if (npc.getNpcId() == 7939) {//rev
            npc.getBehaviour().isAggressive();
            npc.getBehaviour().setRespawn(true);
        }
        if (npc.getNpcId() == 7935) {//rev
            npc.getBehaviour().isAggressive();
            npc.getBehaviour().setRespawn(true);
        }
        if (npc.getNpcId() == 7932) {//rev
            npc.getBehaviour().isAggressive();
            npc.getBehaviour().setRespawn(true);
        }
        if (npc.getNpcId() == 7931) {//rev
            npc.getBehaviour().isAggressive();
            npc.getBehaviour().setRespawn(true);
        }
        if (npc.getNpcId() == 7937) {//rev
            npc.getBehaviour().isAggressive();
            npc.getBehaviour().setRespawn(true);
        }
        if (npc.getNpcId() == 7939) {//rev
        ...
        if (npc.getNpcId() == 7934) {//rev
            npc.getBehaviour().isAggressive();
            npc.getBehaviour().setRespawn(true);
        }
        if (npc.getNpcId() == 7933) {//rev
            npc.getBehaviour().isAggressive();
            npc.getBehaviour().setRespawn(true);
        }
        if (npc.getNpcId() == 6609) {//calisto
            npc.getBehaviour().isAggressive();
            npc.getBehaviour().setRespawn(true);
        }
       ...
       if (npc.getNpcId() == 4420) {// Tournament
        ...
        Player spawn = PlayerHandler.players[npc.spawnedBy];
        if (npc.getNpcId() == VorkathConstants.AWAKENED_VORKATH_ID && npc.actionTimer <= 0 && spawn.getVorkath() != null) {
            if (spawn.getHeight() == npc.getHeight()) {
                if (spawn.withinDistance(npc)) {
                    npc.facePlayer(spawn.getIndex());
                    npc.setFacePlayer(true);
                    spawn.getVorkath().handleAttack();
                }
            }
        }
    ...
    private void processDeath() {
        if (npc.isDead()) {
            ...
            if (npc.actionTimer == 0 && !npc.applyDead && !npc.needRespawn) {
                ...
                if(npc.getNpcId() == 11278){ // Nex death despawn
                    Nex.despawn();
                }

                if(npc.getNpcId() == 1101){ //Powerful sea snake despawn
                    SeaSnake.despawn();
                }

                if(npc.getNpcId() == 5129){ // Glod
                    EventBossHandler.rewardPlayers();
                    EventBossHandler.specialAmount = 0;
                    EventBossHandler.setCurrentLocation(null);

                }
                if (npc.getNpcId() == 3407 ) {//solak
                    SolakEventBossHandler.rewardPlayers();
                    SolakEventBossHandler.setCurrentLocation(null);
                }

                if (npc.getNpcId() == 6477 ) {//Tarn
                    Tarn.rewardPlayers();
                }


                if (npc.getNpcId() == 11246 && npc.getHealth().getCurrentHealth() <= 0) {//Revenant maledictus enranged
                    RevenantEventBossHandler.rewardPlayers();
                    RevenantEventBossHandler.setCurrentLocation(null);
                }

                if (npc.getNpcId() == 8095 ) {//Galvek
                    GalvekEventBossHandler.rewardPlayers();
                    GalvekEventBossHandler.setCurrentLocation(null);
                }
                ...
                if (npc.getNpcId() == 6009) {
                    npc.actionTimer = 1;
                    npc.startAnimation(-1);
                    npc.gfx0(1005);
                    npc.requestTransform(6008);
                    npc.getHealth().reset();
                    npc.setDead(false);
                    npc.forceChat("You dare challenge my inner soul!!!");
                }
                ...
                npcHandler.handleDailyKills(npc);//daily tasks
                ...
                switch (npc.getNpcId()) {
                  ...
                  //      if (playerOwner != null) {
                   ...
                   //     break;
                    ...
                    case 3833://koranian
                        KoranianEventBossHandler.rewardPlayers();
                        KoranianEventBossHandler.setCurrentLocation(null);
                        break;
