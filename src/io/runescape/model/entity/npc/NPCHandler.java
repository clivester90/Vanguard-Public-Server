package io.runescape.model.entity.npc;
...
import io.runescape.content.achievement_diary.AchievementDiaryKills;
...
import io.runescape.content.combat.formula.rework.MagicCombatFormula;
import io.runescape.content.combat.formula.rework.RangeCombatFormula;
import io.runescape.content.combat.melee.MeleeExtras;
import io.runescape.content.dailytasks.DailyTaskKills;
import io.runescape.content.instances.InstancedArea;
import io.runescape.content.minigames.barrows.brothers.Brother;
import io.runescape.content.minigames.fight_cave.Wave;
import io.runescape.content.minigames.inferno.InfernoWaveData;
import io.runescape.content.minigames.pest_control.PestControl;
import io.runescape.content.minigames.xeric.XericWave;
import io.runescape.content.questing.hftd.DagannothMother;
import io.runescape.content.skills.hunter.impling.PuroPuro;
...
import io.runescape.model.entity.npc.actions.LoadSpell;
import io.runescape.model.entity.npc.data.AttackAnimation;
...
public class NPCHandler {
    ...
    public boolean switchesAttackers(NPC npc) {
        ...
        case 5001://Revent maledictus normal version
        case 11246://Revenant maledictus enranged
        case 3833://Koranian
        case 6299://regular donation boss
        case 3407: // solak
        case 5129: // glod
        case 963: 
        case 965: 
    ...
    public static boolean isFightCaveNpc(NPC npc) {
        ...
        switch (npc.getNpcId()) {
        case io.runescape.content.minigames.fight_cave.Wave.TZ_KEK_SPAWN:
        case io.runescape.content.minigames.fight_cave.Wave.TZ_KIH:
        case io.runescape.content.minigames.fight_cave.Wave.TZ_KEK:
        case io.runescape.content.minigames.fight_cave.Wave.TOK_XIL:
        case io.runescape.content.minigames.fight_cave.Wave.YT_MEJKOT:
        case io.runescape.content.minigames.fight_cave.Wave.KET_ZEK:
        case Wave.TZTOK_JAD: 
            return true;
        }
        return false;
    ...
    private int multiAttackDistance(NPC npc) {
        ...
        switch (npc.getNpcId()) {
         ...
         case 7550://olm rightclaw
         case 7551://olm
         case 7552://olm lefttclaw
            return 25;
                ...
                if (PlayerHandler.players[j].goodDistance(c.absX, c.absY, npc.absX, npc.absY, multiAttackDistance(npc))) {
                    ...
                    if (npc.getAttackType() == CombatType.SPECIAL) {
                        ...
                        if (Boundary.isIn(npc, Boundary.NEX_BOSS_ROOM)) {
                            if (!Boundary.isIn(c, Boundary.NEX_BOSS_ROOM)) {
                                return;
                            }
                        }
                        ...
                        if (npc.getNpcId() == 319 || npc.getNpcId() == 3409) {
                        ...
                        if (!c.protectingMagic()) {
                            ...
                            case 1097://powerful snea snake Minion
                            case 1101://powerfull sea snake
                            case 11278://NEX
                                if (c.protectingMagic()) damage /= 2;
                                if (c.protectingMelee()) damage /= 2;
                                if (c.protectingRange()) damage /= 2;
                                break;
    ...
    public void attackPlayer(Player c, NPC npc) {
        ...
        if (npc != null) {
            ...
            if (npc.getNpcId() == Nex.NEX_ID) {
                if (c.getNex() != null) {
                    if (c.getHeight() == npc.getHeight())
                        c.getNex().handleAttackTest(c);
                }
            }

         //   if (npc.getNpcId() == VorkathConstants.AWAKENED_VORKATH_ID) {
         ...
         //       }
          //  }
            ...
            if (Boundary.isIn(npc, Boundary.NEX_BOSS_ROOM) ^ Boundary.isIn(c, Boundary.NEX_BOSS_ROOM)) {
                npc.setPlayerAttackingIndex(0);
                npc.facePlayer(0);
                npc.underAttack = false;
                npc.randomWalk = true;
    ...
    public int getMaxHit(Player c, NPC npc) {
        ...
        switch (npc.getNpcId()) {
            ...
            case 11283:
            case 11284:
            case 11285:// nex minions
            case 11286:
                return 29;

            case 11278://nex
                return npc.getAttackType() == CombatType.MAGE ? 25 : 30;

        case 3833://Koranian
            return npc.getAttackType() == CombatType.RANGE ? 40 : 35;
        ...
        case 3407://solak
            return npc.getAttackType() == CombatType.MELEE ? 45 : 28;

        case 1377://ARKS BOSS
            return npc.getAttackType() == CombatType.MELEE ? 20 : 15;
        case 22545: 
            return 22;
        case 7706: 
            return 120;
     ...
     * Teleport the player next to this npc. Used for barrows
     * example.
     * <p>
    ...
    void handleDailyKills(NPC npc) {
        ...
        if (player != null) {
            DailyTaskKills.kills(player, npc.getNpcId());
        }
    }
