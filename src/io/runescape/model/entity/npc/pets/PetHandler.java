package io.runescape.model.entity.npc.pets;
...
//        ANTI_SANTA_JR(9959, 1048, "Anti-Santa", 500,"first"),
  ...
  //      DARK_CORRUPT_BEAST(30120, 2311, "Dark Corrupt Beast", -1, "second"),
   ...
   //     ARK_DARK_ROC(30129, 2318, "Arks Dark Roc", -1, "second"),//CUSTOM PET FOR ARK
        DARK_KRATOS(30122, 2313, "Dark Kratos", -1, "second"),
    //    DARK_RED_SEREN(30123, 2310, "Dark Seren", -1, "second"),


        MINI_SOLAK(16011, 3406, "Mini Solak", -1, "first"),

        DHAROK_PET(16015, 6479, "Dharok Pet", -1, "first"),
        MINI_GALVEK(16017, 6480, "Mini Galvek", -1, "first"),

        NEXLING(26348, 11276, "Nexling", -1, "second"),

        PET_SHARK(7993, 1830, "Pet Shark", -1, "first"),

        ABYSSAL_PROTECTOR(26901, 11402, "Abyssal Protector", -1, "second"),

        MINI_MALEDICTUS(16012, 5000, "Mini Maledictus", -1, "first");
    ...
    public static Pets getPetForParentId(Pets pet) {
        switch(pet.parent) {
            ...
            case "Dharok Pet":
                return Pets.DHAROK_PET;

            case "Mini Maledictus":
                return Pets.MINI_MALEDICTUS;
    ...
    public static boolean pickupPetNew(Player player, int npcType, boolean item) {// For the new way to pickup pets C.T
        Pets pets = forNpc(npcType);
        if (pets != null) {
            int itemId = pets.itemId;
            if (player.getItems().freeSlots() > 0) {
                NPCHandler.despawn(npcType, player.getHeight());
                player.startAnimation(827);
                player.getItems().addItem(itemId, 1);
                player.petSummonId = -1;
                player.hasFollower = false;
