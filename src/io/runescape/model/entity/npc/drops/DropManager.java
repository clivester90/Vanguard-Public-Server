package io.runescape.model.entity.npc.drops;
...
import io.runescape.content.bosses.grotesqueguardians.GrotesqueInstance;
...
import io.runescape.content.trails.RewardLevel;
...
public class DropManager {
    ...
    public static void dropCoinBag(Player player, int npcId, int dropX, int dropY, int dropZ) {
        ...
        if (Misc.random(8) == 5) {
            ...
            boolean hasDarkVersion = (player.getCoinBagPerk() || player.petSummonId == 30122);
            ...
            || (hasDarkVersion)) {
                ...
                player.sendMessage("@red@Bonus perk:@bla@ you find a @blu@coin bag!");
    ...
    public static void dropResourcePack(Player player, int npcId, int dropX, int dropY, int dropZ) {
        ...
        if (npcDefinition != null && Misc.random(20) == 1) {
           ...
           // boolean hasDarkVersion = (player.petSummonId == 30112 || player.petSummonId == 30122);
            boolean hasDarkVersion = (player.getResourceBoxPerk()|| player.getResourceBoxPerk());
            int extraPack = 0;
            if (player.hasFollower &&
                    ((player.petSummonId == correctPetId || player.petSummonId == 30022) && petPerkChance < 80)
            || (hasDarkVersion)){
                ...
                player.sendMessage("@red@Bonus perk:@bla@ you auto pickup a @blu@resource box!");
    ...
    public void handle(Player player, NPC npc, Location3D location, int repeats, int npcId) {
        ...
        if (Misc.random(chance) == 1) {
           ...
           // boolean hasDarkVersion = player.petSummonId == 30111 || player.petSummonId == 30122;
            boolean hasDarkVersion = player.getClueScrollPerk() || player.petSummonId == 30122;
            int extraKey = 0;
            if (player.hasFollower &&
            ...
            || (hasDarkVersion)) {
                if (player.getItems().freeSlots() > 0) {
                    ...
                    player.sendMessage("@red@Bonus perk:@bla@ you auto pickup a @blu@clue scroll!");
                    player.getItems().addItem(specialItemId, clueAmount + extraKey);
                } else {
                    player.sendMessage("@red@Bonus perk:@bla@ noticed a @blu@clue scroll,@bla@ but your inventory is full!");
      ...
      //  if (Misc.random(80) == 10 || Server.isDebug()) {
        ...
        if (Misc.random(crystalRandom) == 1) {
            ...
            int extraKey = 0;
            if (player.getCrystalKeyPerk()) {
                if (player.getItems().freeSlots() > 0) {
                    player.sendMessage("@red@Bonus Perk: @bla@you notice a @blu@crystal key!");
                    if (player.getCrystalKeyPerk() && petPerkChance < 25) {
                        extraKey = 1;
                    }
                    player.getItems().addItem(specialItemId, keyAmount + extraKey);
                } else {
                    player.sendMessage("@red@Bonus Perk: @bla@You noticed a @blu@crystal key,@bla@ but your inventory is @red@full!");
        ...
        /**
         * Exp lamps for rogue game modes only
         */
        if (Boundary.isIn(npc, Boundary.WILDY_SLAYER_CAVE) || Boundary.isIn(npc, Boundary.WILDY_SLAYER_CAVE_2)
                || Boundary.isIn(npc, Boundary.GODWARS_BOSSROOMS)
                || Boundary.isIn(npc, Boundary.NEX_BOSS_ROOM)
                || Boundary.isIn(npc, Boundary.VORKATH)
                || Boundary.isIn(npc, Boundary.SLAYER_TOWER_BOUNDARY)
                || Boundary.isIn(npc, Boundary.REV_CAVE)) {

            //if (Misc.random(100) == 1) {
            if (Misc.hasOneOutOf(5) && player.getMode().is5x()) {
                   Server.itemHandler.createGroundItem(player, 2528, location.getX(), location.getY(), location.getZ(), 1, player.getIndex());
                   player.sendMessage("@red@You notice some funny looking lamp on the ground.");
            }
        }
    ...
    public void create(Player player, NPC npc, Location3D location, int repeats, int npcId) {
        if (npcId == Npcs.THE_MIMIC_2) {
            if (player.getItems().playerHasItem(Items.MIMIC)) {
                ...
                List<GameItem> drops = player.getTrails().generateRewardList(RewardLevel.MASTER, 2);//used to be 6
    ...
    private static double getModifier(Player player) {
        double modifier = 1.0;
        ...
        if (player.getItems().isWearingItem(2572)) {
            ...
            modifier -= .08;
        } else if (player.getItems().isWearingItem(21753)) {//ring of Vanguard(i)
            modifier -= .12;
        }
        ...
        } else if (player.getDropRateBonus()) {//10% droprate bonus perk
            modifier -= .10;

        } else if (player.getDropRateBonus20()) {//20% droprate bonus perk
            modifier -= .20;
        ...
        } else if (player.getItems().playerHasItem(16015) || (player.hasFollower && (player.petSummonId == 16015))) {//Dharok pet
            modifier -= .10;
        }

        else if (player.getItems().playerHasItem(30129) || (player.hasFollower && (player.petSummonId == 30129))) {//ARKS ROC
            modifier -= .40;
        }
        ...
        if (player.getItems().isWearingItem(34037)) {//death cape
            modifier -= .07;
        }

        //if (Wogw.DOUBLE_DROPS_TIMER > 0)//CAM COME BACK TO THIS
           // modifier -= 0.2;
    ...
    public static double getModifier1(Player player) {
        ...
        } else if (player.getItems().playerHasItem(30129) || (player.hasFollower && (player.petSummonId == 30129))) { // orange rock
            modifier += 40;
    ...
    public void onDrop(Player player, GameItem item, int npcId) {
        ...
        if (WorldEvent.eventStartedAnnounced) {//World event add drops to bank
            if (Boundary.isIn(player, Boundary.WILDERNESS_PARAMETERS))
                player.getItems().addItemToBankOrDrop(item.getId(), item.getAmount());
        }

        if (player.hasFollower && player.petSummonId == 16012) {//Revent maledictus
            if (Boundary.isIn(player, Boundary.REV_CAVE))
                player.getItems().addItemToBankOrDrop(item.getId(), item.getAmount());
        }


        if (player.playerEquipment[Player.playerAmulet] == 22557) {
            if (Boundary.isIn(player, Boundary.WILDY_SLAYER_CAVE)
                    || Boundary.isIn(player, Boundary.WILDY_SLAYER_CAVE_2)
                    || Boundary.isIn(player, Boundary.REV_CAVE))
                // Amulet of avarice
            if (item.getId() == 1201) {//Rune kite
                item.changeDrop(1202, item.getAmount());//rune kite noted
                player.sendMessage("@red@Your amulet has managed to note the rune kite.");
            }
            if (item.getId() == 1163) {//rune helm
                item.changeDrop(1164, item.getAmount());//rune helm noted
                player.sendMessage("@red@Your amulet has managed to note the rune helm.");
            }
            if (item.getId() == 1127) {//rune body
                item.changeDrop(1128, item.getAmount());//rune body noted
                player.sendMessage("@red@Your amulet has managed to note the rune body.");
            }
            if (item.getId() == 1079) {//rune leg
                item.changeDrop(1080, item.getAmount());//rune leg noted
                player.sendMessage("@red@Your amulet has managed to note the rune legs.");
            }
            if (item.getId() == 1347) {//rune warhammer
                item.changeDrop(1348, item.getAmount());//rune warhammer noted
                player.sendMessage("@red@Your amulet has managed to note the rune warhammer.");
            }
            if (item.getId() == 4087) {//Dragon leg
                item.changeDrop(4088, item.getAmount());//Dragon leg noted
                player.sendMessage("@red@Your amulet has managed to note the dragon legs.");
            }
            if (item.getId() == 4585) {//dragon skirt
                item.changeDrop(4586, item.getAmount());//dragon skirt noted
                player.sendMessage("@red@Your amulet has managed to note the dragon skirt.");
            }
            if (item.getId() == 1215) {//dragon dagger
                item.changeDrop(1216, item.getAmount());//dragon dagger noted
                player.sendMessage("@red@Your amulet has managed to note the dragon dagger.");
            }
            if (item.getId() == 1305) {//dragon longsword
                item.changeDrop(1306, item.getAmount());//dragon longsword noted
                player.sendMessage("@red@Your amulet has managed to note the dragon longsword.");
            }
        }
...
//		}
        ...
        if (!RingOfKoranes.collectLoot(player, item.getId(), item.getAmount())) //Ring of koranes
            if (!RingOfKoranesImbue.collectLoot(player, item.getId(), item.getAmount())) //Ring of koranes(i) with the drop rate boost
